 ...
 *
 * **これは警報であって門ではない。** 赤い `main` が `release/prod` へ流れるのを
 * 止めるのは別の仕掛け（`.github/scripts/reflect-release-prod.sh` の CI の門）で、
 * こちらは**その門が止めたことも含めて**知らせる側に回る。⟹ 2つは役割が違う。
 * **片方だけで「塞いだ」と言わないこと** —— 止めるだけだと止まったことが誰にも
 * 見えず、知らせるだけだと誰も見ていない夜に流れる。
 *
 ...
 * 2. 「いま赤いか」を見て振る舞う必要が在るのは**門の側**（夜の反映）であって、
 *    あちらは毎晩読み直す。⟹ 警報 Issue が現在地の正本である必要が無い
 *
