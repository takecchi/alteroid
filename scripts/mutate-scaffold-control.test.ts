...
describe('mutate.mjs run: 足場の赤を差し引いて判定する（端から端まで）', () => {
  ...
  it('🔴 spec に「既知の失敗」を宣言する項目を書いても無視される（人が宣言できる形が無い）', () => {
    ...
    // それらしい名前の項目を足しても判定は1文字も動かない。
    //
    // **ここが弱くなると何が起きるか（#1087。向きを取り違えないこと）。** ここには
    // 「宣言して「検出」を消せる（＝判定を甘くできる）」と書いてあったが、**向きが
    // 逆だった。** 「検出」が消えた先は `生存`（＝歯が弱いという悪い知らせ）で、
    // 偽造する動機が無い側である。危ないのは、**差し引く集合が実測ではなく宣言に
    // なった時点で、その集合の広さを人が握る**ことのほうで、都合のよい向きは
    // **狭く宣言する**側 —— 印に起因する赤が差し引かれずに `surviving` へ残り、
    // **変異を1文字も当てていない状態の赤が「検出」に化ける。** 向きの根拠は
    // `mutate-core.mjs` の `decideJudgementCategory` の doc の「「判定が甘い」の
    // 向き」（`生存` は門を1つも通らずに出せるのに、`検出` は門を4つ通らないと
    // 出ない、という非対称）。
    // **この歯が測っている主張は1文字も変えていない** —— 4鍵は依然として無視される。
    //
