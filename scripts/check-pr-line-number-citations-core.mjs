 ...
 *    その**全件がインラインのバッククォートで囲まれていた**。実例2件（逐語を
 *    そのまま地の文に書くと、この doc 自身が `scripts/agents-md-references.test.ts`
 *    の `path:行番号` 出典検査に引っかかるので、フェンスで囲んで見せる）:
 *    ```
 *    PR #1205: `scripts/test.mjs:118`（本番経路）
 *    PR #1001: `packages/core/src/schema.ts:532,556`
 *    ```
