 ...
 *   結局どちらも CI に配線できなくなる。
 *   ⚠️ **そして「secret に administration のトークンを置けば配線できる」へ逃げない
 *   —— 2026-09-23 に「置かない」と決めた**（#1320。クローンの判断
 *   であって、人間のオーナーの回答ではない）。この repo の `main` は
 *   `enforce_admins: true` で、administration スコープは protection の「読む」と
 *   「書く」を分けられない ⟹ 読むために置いたトークンが、その保護を外せる鍵になる。
 *   **環1は、権限を既に持っている側（クローン）が `pnpm check:required-status-checks`
 *   を定期的に打つことで回す。**
 *
