 ...
 * 接続先を受け取らない（本(4)/歯5）」（3本）は、**恒真テストではない。守って
 * いる範囲がパラメータ名に縛られているだけである。** 両方とも変異試験で測った
 * （実測 2026-09-09、`readStored()` に外部入力を読む実装を足す変異）:
 *
 * - `?apiBaseUrl=` / `#apiBaseUrl=` を読む変異 ⟹ **歯5の3本がちょうど落ちる。**
 *   落ちたのはその3本だけで、他は1本も落ちなかった（`Tests 3 failed | 5115 passed`）。
 *   ⟹ あの3本は「読んでいないこと」を確かに測っている。**消さないこと。**
 * - `?endpoint=` を読む変異 ⟹ **歯5の3本は緑のまま通る。**
 *   このファイルの (B) だけが落ちた（`Tests 1 failed | 25 passed`、2ファイル実行）。
 *
 * ⟹ **つまり歯5が測っているのは「その名前では受け取らない」までで、この歯が
 * 引き継ぐのは「どんな名前でも受け取らない」である。** 前者を後者と読むと、
 * 名前を変えただけの実装が素通りする。**2つは重なっておらず、両方要る。**
 *
