...
**いま守っているのはログイン認証だけである。** アクセストークンはデーモン側の DB にあり、`ALTEROID_GOOGLE_CLIENT_ID` / `_SECRET` は app の Service 変数にしか置かない（`setup.sh` の `app_pairs`）ので、**マネージャーにはトークンを手に入れる経路が無い**。`/auth/*` は開いているが、その先は Google のアカウントと人間の `access grant` を通らないと何も持てない（許可されるアカウントの数に上限は無いが、通すのは人間である。2026-09-09 のオーナー決定まで高々1つだった）。制御面（runner API）の3枚とは別の話で、そちらは上の1のとおり保たれている。
...
### 1. マージした瞬間には落とさない（`release/prod` へ夜に1回写す）

Railway に見せる枝を `main` ではなく **`release/prod`** にする（**向き先を切り替えるのは人間の作業である** — 下の⚠️）。切り替わっていれば `main` へマージしても何も起きず、`.github/workflows/release-prod.yml` が **1日1回、夜だけ**（`cron: '17 19 * * *'` ＝ UTC 19:17 ＝ **JST 翌 4:17**。**⚠️ 2026-09-05 に2回から1回へ減った**。`088a2b2`）`main` と `release/prod` の SHA を比べ、違えば `release/prod` を `main` の先端へ動かす。**設定どおりの時刻には発火せず、実測で 13分〜2時間37分 遅れる**（2026-09-07 観測、`event=schedule` の8回。⟹ 実際は JST 04:29〜06:54）。**デプロイが走る時刻を決めているのはこのワークフローであって、マージした時刻ではない。**
...
**⚠️ そして同じ `runner_id` が2つ並ぶと、いまも二重実行の窓が残る。** 名簿に同名の行が2つあるとき、デーモンは「いまその名前に応えているのがどちらのプロセスか」を決められない。**引き取りだけは止まる** — `packages/core/src/manager.ts` の `#sighting` が台数を返し、`packages/core/src/lease.ts` の `judgeLease` が `ambiguous`（「判定材料が無い」を意味する `undecidable` とは**別の答え**）へ倒し、`mayClaim` がそれを許可の一覧に載せていないので引き取りは通らない（#200 の段1。#209 で入った）。**だが止まるのは「奪わない」までである。次の4つは残っている:**

- **締め出されたはずのプロセスは自己失効しない。** デーモンが両方へ生存確認を打ち続ける限り、貸し出し期限の時計は認証済みの接触1本で延びる。**止めるのは片側だけでは言えない**ので、ここは自動では塞がない（#200 の設計提案「塞げない部分」）
- **話しかける相手は、いまも線形一致で先に見つかった方である**（`packages/core/src/runner-protocol.ts` の `Registry#get`）。**判定した相手と食い違いうる**（#200 の段2。未着手）
- **貸し出しの記録が無い委譲（`unheld`）は、併存でも従来どおり通る。** `judgeLease` は併存の判定より**前**に `unheld` を返すためで、その委譲では2本目が起きうる（#209 が「残る穴」として明記した）
- **名簿そのものは重複を検出しない。** `Registry#register` は `label` をキーに載せるだけで、`runnerId` は繋がるまで分からない（#200 の段3。未着手）

つまり **`ALTEROID_RUNNER_ID` の置き忘れ（既定は `runner-primary` なので2台目で重複する）は、単なる名前の衝突ではなく二重実行の窓である。** 台数を増やすときは `./railway/scale-runners.sh` を通すこと（id を自分で振る）。

**そして runner が N 台あると、runner のコード変更は N 台を同時に畳む。** `watchPatterns` に当たる変更を `release/prod` へ反映すると、runner の Service がそれぞれデプロイされる。1台構成のときより落ちる仕事の数が増えるということで、**台数を増やすことは「デプロイの事故の規模を増やすこと」でもある**（減らないのは委譲が待たされる時間だけである）。
...
**⚠️ そして写しはもう1つある — `packages/core/src/lease.ts` の `LEASE_DRAIN_MS`。** 引き取る側（デーモン）が「旧プロセスはもう畳まれた」と判定するのに使う猶予で、**ここの `drainingSeconds` の写しである。** あちらからこちらへの導線は既に在り（`grep -Fn -- 'ここにあるのは写しである' packages/core/src/lease.ts`）、**この1行が逆向きである。** **危ないのは延ばす向きである** — ここを 60 より長くして `LEASE_DRAIN_MS` を置き去りにすると、器がまだ古いプロセスを畳んでいない時刻にデーモンが先に「畳まれた」と判定し、**その差がそのまま二重実行の窓になる。** 縮める向きは引き取りが遅れるだけで、窓は開かない。判定が器との約束に乗っている（＝片側だけで言える独立な材料ではない）ことは `grep -Fn -- 'は器との約束に乗っている（独立な材料ではない）' packages/core/src/lease.ts` が書いている。
...
**55秒あれば完走する、とも読まないこと。** 60 は外枠、55 は自分で決めた見切りであって、どちらも「蒸留1本が終わる」保証ではない。**猶予を延ばすことは、落ちる前に取りこぼす仕事を減らすだけで、無くしはしない**（**受信箱そのものは器へ残る** — `Clone#post` が `stores.inbox.put` を通し、次の起動で `#restoreUnread` が本文ごと配り直す。**それでも `post()` は同期で返り、書き込みの完了は待たない**ので、受理から書けるまでの一瞬に消えた合図は戻らない。この窓が塞げないことは `clone.ts` の `post()` の doc が自分で書いている）。compose 側（`compose.yaml` の `stop_grace_period: 60s`）も同じ 60 で、同じ 55 が効く。
...
- **runner が入れ替わったら、走行中だった仕事を取り直す。** runner の SSE は繋ぎ直すたびに名乗る（`hello`）。名乗りを聞いたら `list()` を突き合わせ、器の中に居なくなっていたものを `session_id` と預かった生ログから resume する。ストリームが一瞬切れただけなら `list()` にそのまま並ぶので何も起きない
  ...
  - **⭐ 取り直しは `drainingSeconds` の猶予の中では走らない**（貸し出し期限。`packages/core/src/lease.ts`）。上の `drainingSeconds` が意味するのは「畳まれつつある旧 runner はまだ手を動かしている」ことなので、入れ替えを見た瞬間に resume を出すと**旧 runner のマネージャーと合わせて同じ仕事が2本走る**（`gh pr create` が二度走る形）。だから台帳に前のプロセスの貸し出しが残っている委譲は、`drainingSeconds` + 余裕30秒を過ぎてから引き取る — **入れ替えの直後にすぐ動かないのは、忘れているのではなく待っているのである**（待っている事実は日誌に判断として残る）
    - **⚠️ 再デプロイでは待つ。** 「行儀よく畳めば `closed` が来るから待たない」と読みたくなるが、**SIGTERM で畳む経路（`host.shutdown()` → `session.stop()`）は `closed` を上げない**（`packages/core/src/runner.ts` の `stop()` は意図的に黙る — 畳めと言った側が結果を `list()` で確かめられるからである）。だから**通常の再デプロイでは貸し出しは返らず、猶予（60秒 + 余裕30秒）を待ってから引き取る。** 待たずに済むのは `closed` を上げる経路（クラッシュ・resume 不能・自己失効・そのターンを終えた `done`）だけである
      - **これは欠陥ではなく、待つ側が正しい。** 畳む猶予の間、旧 runner はまだ手を動かしている（それが `drainingSeconds` の意味である）。**そこで引き取ると、旧 runner のマネージャーと合わせて同じ仕事が2本走る**
    - **旧 runner の側にも歯がある。** 認証済みの制御面の接触（`/health` の名乗りを含む）が途切れたまま貸し出し期限（既定10分）を過ぎると、runner は走行中のセッションを**自分で畳む**。これは器が旧プロセスを殺さない構成（経路だけが付け替わった等）のための最後の口で、`drainingSeconds` の側が先に効く構成では出番が無い。**既定を daemon 自身の入れ替えより十分に長くしてあるのは、daemon だけを再デプロイしている間に走行中のマネージャーを殺さないためである**
...
| 公式の Web UI から叩けるようにするか（CORS）                    | CORS ヘッダを返さない（＝デーモンの既定のまま）   |

**3つ目だけ既定が「はい」である。** 他の2つは人間が鍵を持ってこないと成立しないが、これは何も要らないうえ、外から画面で見るのがいちばん多い使い方だからである。**それでもデーモンのコードの既定は閉じたままにしてある**（`docs/architecture.md`「CORS は既定で閉じている」）— 開けるならその事実が `.env` と Service 変数に残るべきで、コードの既定に焼くと**開いていることがどこにも現れない**。閉じたまま建てるなら、この質問に「いいえ」と答えるか `.env` に自分の列挙を書いておく。
...
| `ALTEROID_ALLOWED_ORIGINS` | `https://alteroid.vercel.app` 等 | ブラウザから叩いてよいオリジンの明示列挙。**読むのはデーモンだけで、runner は1度も見ない。** `setup.sh` が尋ねて置く（答えが「いいえ」なら作らない） |
