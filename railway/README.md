...
main へマージすると GitHub 連携が動く。**器の入れ替えは、その中で手を動かしているマネージャーと作業者を畳む**ということなので、無関係な変更で起こさない。3つで抑えてある。

### 1. その Service が動かすものが変わったときだけ落とす（`watchPatterns`）

`railway/daemon.json` / `railway/runner.json` の `build.watchPatterns` に、その Service が実際に動かすパスだけを並べてある。`docs/` の更新、この README、`compose.yaml`、CI の変更では**どちらも落ちない**。`packages/**` は両方が動かすので、core が変われば両方が上がり直す（それは正しい）。

`apps/cli/**` は `app` にだけ入れてある。`alteroid chat` が走るのは daemon 側であって、runner ではない（runner のイメージには古い `apps/cli/dist` が残るが、そこで誰も実行しない）。
...
パスを増やしたときは watchPatterns も直すこと。**漏らすと「直したのに反映されない」になる**ので、迷ったら広めに入れる（余分なデプロイは、届かない修正よりましである）。

`overlapSeconds` は使っていない。新旧の runner が同じ `ALTEROID_RUNNER_ID` で並ぶと、台帳の `manager_id → runner_id` がどちらを指すのか決まらない。

### 2. 畳む時間を渡す（`drainingSeconds: 60`）
...
- `app`: 落ちる前の**最後の蒸留**（クローンのターン1本）が終わるまでの時間。足りないと記憶へ移すはずだったものが消える

### 3. 落ちた側を待つ / 取り直す

- **daemon は runner を待つ**（最大2分、指数バックオフ）。以前は起動時に繋がらないと即 `exit(1)` していたので、同時デプロイのたびにクラッシュループしていた。落ちている間はクローンのターンも `/approvals` への回答も受け付けられない。鍵が無い・鍵が違うといった**方針の誤りは待たずに落ちる**（待っても直らない）
- **runner が入れ替わったら、走行中だった仕事を取り直す。** runner の SSE は繋ぎ直すたびに名乗る（`hello`）。2回目以降の名乗りで `list()` を突き合わせ、器の中に居なくなっていたものを `session_id` と預かった生ログから resume する。ストリームが一瞬切れただけなら `list()` にそのまま並ぶので何も起きない

3番目が無いと、**daemon が生き残って runner だけ入れ替わった場合に仕事が誰にも拾われない**（引き取りの契機が daemon の起動時しか無いため）。1番で落ちる範囲を分けた以上、この経路は必須である。人間の不在で止まってよいのは承認待ちの仕事だけである（PRD「自律」）。

**それでも、走行中の仕事にとってデプロイは事故である。** 上の3つは被害を減らすだけで、無くしはしない。長い仕事を投げている最中のマージは避けるか、先に `/managers` で走行中を確かめる。
...
**長い仕事を頼むときは、早めに push させること。** workspace はボリュームを付けていないので、runner が再デプロイされるとコミット前の変更は消える（「先に読む」3）。器が入れ替わっても仕事は取り直され、消えたことも伝わるが（「デプロイは走行中の仕事を畳む操作である」）、**書いたものが戻るわけではない**。数時間かかる仕事を投げるなら、その間マージしないか、`/workspace` にボリュームを付ける。

**クローン自身が PR を出す構成では、ここが輪になる。** マネージャーが出した PR を人間がマージした瞬間にデプロイが走り、まだ走っている別の仕事を畳む。`watchPatterns` で `docs/` だけの PR は落ちなくなったが、コードの PR は落とす。マージ前に `/managers` を見る。
