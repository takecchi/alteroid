...
**runner は1台でも複数台でもよい。** 建てるときに決めるなら `setup.sh --runners <N>`、後から増やすなら `add-runner.sh`（下の「runner を増やす / 減らす」）。**台数が変わっても人間の手順は変わらない** — 変数の置き場も、境界の確かめ方（`verify.sh`）も、台数を数えるのはスクリプトの側である。

**台数を数えているのは Railway 側の Config as Code である**（`/railway/runner.json` を指している Service が runner）。この文書にもスクリプトにも台数は書かない — 書けば増やすたびに直して回ることになり、片方だけ古びる。
...
`overlapSeconds` は使っていない。新旧の runner が同じ `ALTEROID_RUNNER_ID` で並ぶと、台帳の `manager_id → runner_id` がどちらを指すのか決まらない。**同じ理由で、runner のレプリカを2以上にしないこと** — 1つの `runner_id` の裏に複数のプロセスが並ぶと、SSE と `list()` が別のプロセスに当たって取り直しが誤判定する。**runner を増やすのは「別の `ALTEROID_RUNNER_ID` を持つ Service を足す」であって、レプリカ数ではない**（下の「runner を増やす / 減らす」。`add-runner.sh` がその形で足す）。
...
./railway/setup.sh --runners 2   # 最初から2台で建てるならこう
```
...
- プロジェクトを**新しく作る**（既存には触らない）→ PostgreSQL → `app` / `runner`（`--runners N` なら `runner-2` … も）
...
- **`runner` を（全台）先に**上げ、上がってから `app` を繋ぐ
...
**繋ぐ枝は `release/prod` である**（`setup.sh` の既定。`-b` で変えられる）。`main` へ繋ぐと、マージした瞬間に落ちる形へ戻る（「デプロイは走行中の仕事を畳む操作である」1）。**`runner` Service を2つ目足すときも同じ枝に繋ぐこと** — 1つだけ `main` を見ていると、そこだけがマージのたびに畳まれる。`add-runner.sh` は**写し元の runner が見ている枝をそのまま使う**（ローカルの git からも既定値からも決めない）ので、ここを人間が指定し直す必要は無い。
...
roadmap M5「Railway の複数 Service …で runner 数を増減できるデプロイ定義」の Railway 側である。

### 増やす

```bash
railway link                 # 対象のプロジェクトへ紐づく（既存に足す道具なので必須）
./railway/add-runner.sh      # 名前・runner_id・変数・繋ぐ枝は既存から決まる
```

**`setup.sh` は使わない。** あちらは毎回新しいプロジェクトを作る（既存には触らない）ので、動いている環境の台数は増やせない。最初から複数台で建てるときだけ `setup.sh --runners <N>` である。

`add-runner.sh` がやること（`-h` に残りのオプションがある）:

| すること                                                   | なぜそうしているか                                                                                                                                                                       |
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 空いている名前（`runner-2`, `-3` …）を選ぶ                 | 名前は環境の中で1つ。ぶつかると `add` が別の器を作るのか失敗するのかが Railway 側の都合になる                                                                                            |
| **`runner_id` の重複を確かめて、ぶつかったら止まる**       | `RunnerRegistry#get` は線形一致なので、同じ id が2台並ぶと**先に見つかった方**が返る。委譲した先とは別の器へ `manager_send` が届き、しかも届いているように見える（気づく場所が他に無い） |
| 変数を**写し元の runner から**写す                         | `.env` から取ると、`.env` に無い変数（人間が後からダッシュボードで足したもの）が2台目にだけ無い状態になる ＝ 当たった runner でできることが変わる                                        |
| `RAILWAY_*` は写さない（`RAILWAY_RUN_UID` だけ残す）       | Railway が注入する値なので、写すと嘘になる。`RAILWAY_RUN_UID=0` だけは我々が置いている値で、無いと runner は**起動を拒む**                                                               |
| **記憶ストアの鍵・入口の認証の鍵が写し元に在ったら止まる** | 境界が既に壊れているのに黙って落とすと、壊れている事実が出力から消えて2台目だけが健全に見える                                                                                            |
| 繋ぐ枝は**写し元と同じ**                                   | 1台だけ `main` を見ていると、そこだけがマージのたびに畳まれる（上の「デプロイは走行中の仕事を畳む操作である」1）                                                                         |
| `app` の `ALTEROID_RUNNER_URLS` を全台に置き直す           | 名簿は起動時の種である。ここを置き忘れると、上がっているのに委譲先にならない                                                                                                             |
| **`app` を再デプロイする**                                 | **変数を置いただけでは走っているデーモンに届かない**（器の環境変数は起動時に決まる）。黙って飛ばすと「足したのに委譲先が増えない」を Railway 側に探しに行く                              |

**`app` の再デプロイはクローンのターンを数十秒止めるが、走行中のマネージャーは畳まれない**（マネージャーが居るのは runner の中で、そちらは触らない）。デーモンは上がり直したときに走行中だった仕事を取り直す（同上の4）。止めたくないなら `--no-redeploy-app` で、そのときは**次のデプロイまで新しい runner は委譲先にならない**（スクリプトがそう言って終わる）。

確認は `./railway/verify.sh`。**台数は Config as Code から引くので、足した1台も自動で見に行く**（`runner_id` の重複と、「app が知っている委譲先の数」の食い違いもここで落ちる）。

### 減らす

**スクリプトにしていない。** Service を消す操作は、その器の中に走行中の仕事が居るかどうかを Railway 側から判定できず、移送（roadmap M5 PR5）もまだ無いので、**消した瞬間に消えるものがある**。人間が順に確かめる。

1. `alteroid chat` の `/managers`（または `GET /managers`）で、その runner に載っている仕事が無いことを見る
2. 走っているものがあれば、終わるまで待つ（新しい委譲がそこへ行かないようにする手段はまだ無い）
3. ダッシュボードで Service を削除する
4. `app` の `ALTEROID_RUNNER_URLS` から消えた台を外し、`app` を再デプロイする

**4を忘れると、名簿に死んだ宛先が残る。** 委譲そのものは生きている台へ回るが、`GET /runners` に `unreachable` が残り続け、背景の再接続もバックオフしながら挑み続ける。`verify.sh` は「app が知っている委譲先の数 > 見た runner の数」を `!` として出す。
...
**runner を増やすときも同じ `/railway/runner.json` を指す**（`watchPatterns` も `drainingSeconds` も台ごとに変える理由が無い）。**役を決めているのはこのパスであり、名前ではない** — `verify.sh` と `add-runner.sh` は「このパスを指している Service」を runner の名簿として読む。だから名前を変えても数え上げは壊れないが、**パスを指し忘れた Service は名簿に載らないまま静かに落ちる**（そして役が決まらないので何も走らない）。
...
**ただし `ALTEROID_RUNNER_ID` は共有できない**（台ごとに違う値でなければならないので、Service 変数として1台ずつ置く）。共有してよいかどうかは「片方が読まないだけか」ではなく「**全台が同じ値でよいか**」で切る。同じ値だと壊れるのはこれだけである。
...
| `ALTEROID_RUNNER_URL`     | `http://${{runner.RAILWAY_PRIVATE_DOMAIN}}:4518` | 委譲の宛先（app が読む）。固定 URL をコードに埋めず、ここで名簿へ登録する。private network は Wireguard で暗号化済みなので `http://`。**複数台を並べるなら下の `ALTEROID_RUNNER_URLS`**（単数形も引き続き効く）。どちらも起動時の種であって、繋ぐのは待ち受けを開いた後の背景である                                                                  |
| `ALTEROID_RUNNER_URLS`    | 同じ形をカンマ区切りで全台                       | 台数ぶん並べる（`…{{runner.…}}:4518,…{{runner-2.…}}:4518`）。**単数形と重複していても害は無い** — 空白と重複は `parseRunnerUrls` が落とす。`setup.sh` / `add-runner.sh` はここを置き直す                                                                                                                                                             |
...
| `ALTEROID_RUNNER_ID`      | `runner-primary`                                 | 台帳の `manager_id → runner_id` を引く安定した識別子。器を作り直しても同じ宛先として戻る。**台ごとに違う値にする**（同じ id が2台並ぶと、`RunnerRegistry#get` は線形一致なので先に見つかった方が返る ＝ 委譲した先とは別の器へ命令が届く）。**app には置かない** — app は読まないうえ、1つだけ置くと「2台居るのに id は1つ」という嘘が変数一覧に残る |
...
**`runner` を（複数台なら全部）先に上げる。** daemon は起動時に runner の `/health` へ名乗りを聞きに行く（鍵無しで繋ぐくらいなら起動しない設計）。繋がらなければ最大2分は待ち直すので順番を外しても収束するが、順番どおりなら待たずに上がる。
...
**runner は台数ぶん見る。** 見る相手は Config as Code（`/railway/runner.json` を指している Service）から引くので、`add-runner.sh` で足した1台も自動で入る。`-r` を繰り返せば手で指定できる。**引けなかったときは既定の1台だけを見て `!` を1つ立てる** — 「1台だった」と「引けなかった」を同じ顔にすると、足した台が確かめられていないことが出力から消える。

台数が2以上のときだけ見ているものが2つある。**どちらも1台構成では起きえない**ので、増やした後に初めて意味を持つ。

| 確かめること                                     | ずれると何が起きるか                                                                                                                             |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `runner_id` が台ごとに違う（走っている環境から） | 同じ id が2台並ぶと、委譲した先とは別の器へ `manager_send` が届く（`RunnerRegistry#get` は線形一致）                                             |
| app が知っている委譲先の数 ＝ 見た runner の数   | 少なければ **`app` の再デプロイが済んでいない**（変数を置いただけでは走っているデーモンに届かない）。多ければ、消した Service が名簿に残っている |

以下はそれが何をしているかで、手で確かめるときはこれを打つ。境界が本当に立っているかは、思い込みではなく runner の中から確かめる。**複数台あるなら、これを台ごとに打つ**（1台だけ見て「境界は立っている」と名乗らないこと）。
...
**M5 は実装だけでは受け入れ基準を満たさない。** 基準1が「runner を2台以上登録し、複数マネージャーが配置される」なので、`runner` Service をもう1つ（別の `ALTEROID_RUNNER_ID`）足して初めて確認できる。**足す手順は上の「runner を増やす / 減らす」にある**（`./railway/add-runner.sh`）。**ただし「足せる」と「基準を満たした」は別である** — 足したうえで、委譲が両方へ配られること・`manager_send` が割り当て先へ届くことを実際に見るまでは満たしていない（基準2）。実装を頼むときに「1台構成のまま通るところまで」と「2台目を足してからの確認」を分けて伝えると迷子にならない。
...
| runner を足したのに委譲先が増えない（`GET /runners` に出ない）   | **`app` を再デプロイしていない。** 名簿は起動時の種で、器の環境変数は起動時に決まるので、変数を置いただけでは走っているデーモンに入らない（`add-runner.sh --no-redeploy-app` を選んだときもこれ）。`railway service redeploy --service app --yes`。次に `ALTEROID_RUNNER_URLS` に新しい台が並んでいるかを見る |
| 委譲した仕事が、頼んだのとは別の runner の中で走っている         | **`ALTEROID_RUNNER_ID` が重複している。** `RunnerRegistry#get` は線形一致なので、同じ id の2台のうち**先に見つかった方**が返る。`./railway/verify.sh` が `!!` で落とす。台ごとに違う値を置き直して、その Service を再デプロイする                                                                             |
| 足した runner が `verify.sh` に出てこない                        | その Service の **Config as Code が `/railway/runner.json` を指していない**（役の持ち主はこのパスであって名前ではない）。同じ理由で `startCommand` も決まらないので、そもそも何も走っていない                                                                                                                 |
...
4. **Service は全部が常時起動する**（runner を足せばその数だけ増える）。止めてよいのは承認待ちの仕事だけで、器ではない
5. **新しい委譲を特定の runner へ行かせない手段が無い。** だから減らすときは「走っているものが終わるのを待つ」しかない（上の「runner を減らす」）。drain の口は roadmap M5 の fencing / 移送と同じ列にある
