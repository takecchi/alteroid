...
**⚠️ そして同じ `runner_id` が2つ並ぶと、貸し出し期限（fencing）の守りが丸ごと無効になる。** 名簿に同名の行が2つあるとき、デーモンは「いまその名前に応えているのがどちらのプロセスか」を決められないので、判定を「判定できない」へ倒す（＝**引き取りを許す**。`packages/core/src/manager.ts` の `#sighting`）。しかも締め出されたはずのプロセスは、デーモンが両方へ生存確認を打ち続ける限り**自己失効もしない**（貸し出し期限の時計は認証済みの接触1本で延びる）。つまり **`ALTEROID_RUNNER_ID` の置き忘れ（既定は `runner-primary` なので2台目で重複する）は、単なる名前の衝突ではなく二重実行の窓である。** 台数を増やすときは `./railway/scale-runners.sh` を通すこと（id を自分で振る）。
