...
### 1. マージした瞬間には落とさない（`release/prod` へ6時間ごとに写す）

Railway に見せる枝を `main` ではなく **`release/prod`** にする（**向き先を切り替えるのは人間の作業である** — 下の⚠️）。切り替わっていれば `main` へマージしても何も起きず、`.github/workflows/release-prod.yml` が **6時間ごとを目安に** `main` と `release/prod` の SHA を比べ、違えば `release/prod` を `main` の先端へ動かす。**デプロイが走る時刻を決めているのはこのワークフローであって、マージした時刻ではない。**
...
`cron` は**設定どおりには発火しない。** 別のリポジトリでの実測で、30分間隔の設定に対して実際の間隔は25〜139分だった。ここも6〜12時間へ伸びうる。**目的は「最低6時間は空ける」なので遅れる方向は困らない**が、「6時間ごとに必ず走る」とは読まないこと。待てないとき（本番が壊れている・直した欠陥をいま確かめたい）は Actions から `workflow_dispatch` で起こす。
...
**それでも、走行中の仕事にとってデプロイは事故である。** 上の4つは被害を減らすだけで、無くしはしない。1番で消えたのは「**自分の**マージで死ぬ」だけで、**畳まれる契機がマージから6時間ごとの反映へ移った**にすぎない。しかも移った先は**自分の操作と無関係な時刻**なので、「いま落ちない」と言える瞬間は前より減っている。長い仕事は早めに push させる（下の「頼む」）。
...
**クローン自身が PR を出す構成では、ここが輪になっていた。** マネージャーが出した PR を人間がマージした瞬間にデプロイが走り、まだ走っている別の仕事を畳む — 出した本人ごと畳まれることもあった（2026-08-18、#73）。**`release/prod` を6時間ごとに写す形にしてこの輪は切れている**（「デプロイは走行中の仕事を畳む操作である」1）。**マージしても、その瞬間には何も落ちない。** ただし落ちる時刻が自分の操作から離れただけなので、`/managers` を見る意味は消えていない — 見るのが「マージする前」から「長い仕事を投げる前」に変わった。
