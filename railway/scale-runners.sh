...
# ## 減らすほうはやらない
#
# `-n` に今より小さい数を渡したら**断る**（非0で終わる）。台数を減らす操作は、その器で
# 走っているマネージャーを移送できて初めて安全になり、移送は fencing（貸し出し期限）の
# 後でしかできない（roadmap M5 PR4 → PR5）。**「落ちた」は観測の欠落であって停止の
# 証明ではない**ので、いま減らすと同じセッションが2か所で走りうる。
...
if [ "$TOTAL" -lt "$CURRENT" ]; then
  ...
  die "$CURRENT 台から ${TOTAL} 台へは減らせない（このスクリプトは増やすだけ）。
    台数を減らすには、その器で走っているマネージャーの移送が要る（roadmap M5 PR4 → PR5）。
    いま手で消すなら、その器に仕事が無いことを先に確かめること:
