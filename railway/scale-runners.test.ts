...
/**
 * **`runScriptAsync` である（`runScript` ではない）。** `it` の中のプロセス起動を
 * 追い出した後（#1100）、この足場が組み立てる8シナリオを `prepareScenarios` が
 * `runLimited` で並行に走らせるためには、非同期でなければならない。
 */
 ...
 * 測った（それぞれ2回、`Date.now()` の差分。`setup.test.ts` の
 ...
 * | ----------------------------------------- | ------------------ |
 * | 無負荷                                     | PLACEHOLDER        |
 * | `nproc`（32本）の CPU busy-loop を掛けた状態 | PLACEHOLDER        |
 * | `taskset -c 0,1`（2芯に絞った状態）        | PLACEHOLDER        |
 * | 2×`nproc`（64本）の busy-loop を掛けた状態  | PLACEHOLDER        |
 *
 * **最悪値は PLACEHOLDERms。** `taskset -c 0,1` は `os.cpus()` が返す論理コア数を
 * 変えない（affinity だけを絞るため）ので、`runLimited` は2芯の器でも32本ぶん
 * 並行に投げにいく——**芯数が少ない器ほど「並行度を実コア数より高く見積もって
 * 溢れる」側の最悪ケースに近い**、という点で上の4条件のうち最も実運用の悪条件に
 * 近いと考えている（`setup.test.ts` と同じ考え方）。
 *
 * **倍率は最悪値の約2.5倍。** 単発の測定なのでばらつきが在り、その振れ幅を
 * 考えると2.5倍は「もう1段階悪い器」を吸収できる程度の余裕として選んだ——時間を
 * 無限に伸ばして問題を隠す発想ではなく、実測した最悪値を起点にしている
 * （`setup.test.ts` の `PREP_TIMEOUT` と同じ倍率を採用）。
 */
