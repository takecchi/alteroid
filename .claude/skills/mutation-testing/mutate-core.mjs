 ...
 * 直接引くほうが確実である。
 *
 * ## 「判定が甘い」の向き（#1087。ここが正典。他の記述はここを指す）
 *
 * **甘い＝「検出」へ倒れる側である。逆ではない。** リポジトリ内の記述が逆向きに
 * 読める形で散っていたので（#1087）、向きをここで1度だけ決める。根拠は3つ。
 *
 * 1. **欲しがられている答えが「検出」だからである。** 変異試験を撃つ人が得たい
 *    結論は「狙った歯は効く」＝ `検出` であって、`生存` は「歯が弱い」という悪い
 *    知らせである。**得やすくしてはいけないのは「検出」のほうである。**
 * 2. **上の門の表が、それを構造で示している。** 並んでいる門のうち片側だけを塞ぐ
 *    ものは**全部「検出」の手前**に在る —— `生存` の2本の return（「すべて緑」と
 *    「差し引いた残りが空」）は門を1つも通らずに出るのに、`検出` は門4・(5)・6・7
 *    を全部通らないと出ない（門1〜3 はどちらの側も等しく拒むので数えない）。
 *    **無条件に出せる側と、通行証を4枚要求される側。** この非対称そのものが、
 *    このハーネスがどちらを疑っているかの答えである
 * 3. **この repo が別の場所で自分でそう書いている。** 逐語は
 *    `grep -Fn -- '害は安全側（偽の生存）に倒れるはず' .claude/skills/mutation-testing/SKILL.md`
 *    —— **偽の「生存」を「安全側」と名指している。** ⟹ 甘い側は反対の `検出` で
 *    ある（⚠️ その一文が言いたいのは「その前提はこの場面では成り立たない」だが、
 *    **どちらが安全側かという向きの部分は否定されていない**）
 *
 * **⟹ 人が宣言できる形が危ないのは、「宣言で『検出』を消せる」からではない。**
 * 消える先は `生存` なので、そちらは厳しい側である（それでも本物の検出が
 * 差し引かれた集合に隠れうるので、差し引いた名前は必ず列挙する ——
 * `formatScaffoldSubtractionReport`。**塞ぎ方が「拒む」ではなく「列挙する」で
 * 足りている**こと自体が、この向きが甘い側ではないことの現れである）。危ないのは
 * **差し引く集合が実測ではなく宣言になった時点で、その集合の広さを人が握る**
 * ことである —— 都合のよい向きは**狭く宣言する**側で、印に起因する赤が差し引かれ
 * ずに `surviving` へ残り、**変異を1文字も当てていない状態の赤が「検出」に化ける**
 * （#749 が塞いだ欠陥そのもの。その commit の題は「変異試験の判定に落ちた歯の
 * 名前を使い、足場の印による偽の『検出』を止める」である）。
 *
 * **⚠️ `mustFail` をこれと混ぜないこと。線は「向き」ではなく「差し引く集合に
 * 触るかどうか」で引く。** `mustFail`（`requireDeclaredTargetTeeth`）は
 * `surviving` が確定した**後**に交差を1度取るだけで赤を1本も差し引かないので、
 * 動くのは `検出` → `身代わり` の側だけ ——**厳しくする側にしか動かない。**
 * 矢印だけで見分けようとすると外れる: **禁じられている形も許されている形も、
 * どちらも `検出` から遠ざかる向きへ倒れる。**
 *
 * **⚠️ 「厳しくする側にしか動かない」には基準が要る。基準は2つあり、#1137 で
 * 片方が壊れていたのを直した。**
 *
 * - **基準1（#993 以前の件数ベースの判定）**: 旧は赤が1本でもあれば `検出`、新は
 *   宣言との交差が要る ⟹ 新 ⊆ 旧。**この基準では最初から成り立っていた。**
 * - **基準2（正直に狙いだけを宣言した場合）**: ⛔ **こちらは成り立っていなかった。**
 *   実測（2026-09-16）で、走行の中身が完全に同一のまま、狙い1本だけの宣言では
 ...
 * 項目を書いても無視される」の歯）。**区別は「差し引く集合に触るかどうか」で
 * あって、矢印の向きではない**（#1087 —— ここを矢印で書いていたのが逆向きの
 * 記述の出どころだった。下の2つは**どちらも `検出` から遠ざかる**ので、矢印だけ
 * では甘い側を見分けられない）:
 * - 禁じられている形: 宣言が**差し引く集合そのものになる**（対照の実測を宣言で
 *   置き換える）＝ **判定を甘くできる口。** 集合の広さを人が握った時点で、都合の
 *   よい向き＝**狭く宣言する**側が開き、印に起因する赤が差し引かれずに残って
 *   **偽の「検出」**になる（向きの根拠は `decideJudgementCategory` の doc の
 *   「「判定が甘い」の向き」）
 * - ここで足す形: 宣言は `surviving` を1本も減らさず、確定した**後**に交差を1度
 *   取るだけ。**`検出` を名乗れる条件を狭めるだけ**（`検出` → `身代わり`）＝
 *   **厳しくする側にしか動かない**
 *
  ...
  if (!declaredOk) {
    throw new HarnessError(
      `落ちた歯が在るが、狙いの歯（mustFail）が宣言されていないので判定を出さない（${contextLabel}）。\n` +
        ...
        '名前の拾い方（⚠️ 拾うのは名前の**形式**であって、**どれを宣言するか**ではない）: ' +
        '一度走らせて、証跡の区画「足場対照との差し引き」に出る' +
        ' `[残った] <名前>` の行から、**狙った歯の行だけ**を写すこと' +
        '（生ログの "FAIL " 行にも同じ文字列が出る）。\n' +
        '⛔ `[残った]` を全部写さないこと（#1137）。落ちていない歯が混ざると門7が判定を拒む' +
        '——「宣言を広く書くほど検出になりやすい」を塞いである。\n' +
  ...
  if (missing.length > 0) {
    throw new HarnessError(
      `${contextLabel}: mustFail に実在しない名前が${missing.length}本ある。判定を出さない。\n` +
        ...
        '正しい名前の拾い方（⚠️ 拾うのは名前の**形式**であって、**どれを宣言するか**ではない）: ' +
        '一度走らせて、証跡の区画「足場対照との差し引き」に出る' +
        ' `[残った]` の行、または生ログの " FAIL " 行から、**狙った歯の行だけ**を写すこと。' +
        '⛔ 全部写さないこと（#1137。落ちていない歯が混ざると門7が判定を拒む）。',
    );
  }
}
 ...
 * `surviving` に居れば 検出」だった。⟹ **宣言を広く書くほど「検出」になりやすい。**
 * 実測（#1137、`decideJudgementCategory` の直接呼び出し）では、**走行の中身が
 * 完全に同一**（狙いの歯 A は census 上 `passed`、実際に落ちたのは無関係な B だけ、
 * 足場対照は赤0本）なのに、宣言の書き方だけで結果が割れた:
 *
 * - `mustFail: [A]`（正直な宣言） → `身代わり`
 * - `mustFail: [A, B]`（`[残った]` をそのまま写した宣言） → `検出`
 *
 * **⚠️ しかもハーネス自身がその書き方を指示していた** —— 門5 / 門6 のメッセージが
 * 「`[残った]` の行をそのまま写すこと」と言っていた。素直に従うと、1度目に赤く
 * なったものを全部宣言することになり、2度目は定義により必ず `検出` になる。
 *
 * **向きを反転させる。** 宣言に「落ちていない歯」を混ぜると、`検出` は**通らなく
 * なる** ⟹ `requireDeclaredTargetTeeth` の doc が言う「厳しくする側にしか動かない」
 * が、**正直に狙いだけを宣言した場合を基準にしても**成り立つようになる。
 *
 * **狭すぎない。** 見るのは部分集合であって一致ではない ——
 * 1つの変異が正当に3本落とすとき、3本とも宣言しても、狙いの1本だけを宣言しても、
 * どちらも空振り0本なので `検出` になる。**壊れるのは「落ちていない歯を混ぜたとき」
 * だけである。**
 *
 * **拒む形は `HarnessError`（判定そのものを出さない）。** 新しい分類語を増やさない
 * のは、門4 / 門6 / `requireDeclaredTeethActuallyPassed` と同じ倒し方に揃えるため
 * である（`AGENTS.md`「判定できないという3つ目の状態を持つ」）。
 */
export function requireNoDeclaredTargetMisses(declaredNames, survivingSet, census, contextLabel) {
  const missed = [...declaredNames].filter((name) => !survivingSet.has(name));
  if (missed.length === 0) return;
  const described = missed.map((name) => {
    const status = isCensusAvailable(census)
      ? (census.byName.get(name) ?? '(census に実在しない)')
      : '(census が取れていない)';
    return `  - ${JSON.stringify(name)}（census 上の状態: ${JSON.stringify(status)}）`;
  });
  throw new HarnessError(
    `${contextLabel}: 宣言した歯（mustFail）のうち${missed.length}本が空振りしている。` +
      '判定を出さない。\n' +
      `空振り（この変異に起因する赤に居ない）:\n${described.join('\n')}\n` +
      'なぜ拒むか: 「検出」は「狙った歯がこの変異を捕まえた」という主張である。' +
      '落ちていない歯が宣言に混ざっていると、その主張の一部が外れていることになる' +
      '——それでも「検出」と名乗れると、**宣言を広く書くほど検出になりやすい**という' +
      '向きが開く（#1137。同一の走行が宣言の書き方だけで 身代わり にも 検出 にもなった）。\n' +
      '次にやること: mustFail から空振りの名前を外すこと。' +
      '⛔ 逆向き（落ちた歯を全部書き足す）で通さないこと——それがこの門の塞いでいる形そのものである。',
  );
}
...
export function decideJudgementCategory(artifactResult, testResult, scaffoldControl, mustFail) {
  ...
  if ([...declared].some((name) => survivingSet.has(name))) {
    requireNoDeclaredTargetMisses(
      declared,
      survivingSet,
      testResult.census,
      'decideJudgementCategory',
...
// **⚠️ 甘くなる向きは「検出」側である**（#1087）—— 宣言が差し引く集合そのものに
// なれば、狭く宣言した分だけ印に起因する赤が `surviving` へ残り、変異を1文字も
// 当てていない赤が「検出」に化ける。**「宣言で検出を消せる」ほうではない**
// （消えた先の `生存` は悪い知らせなので、偽造する動機が無い）。向きの根拠は
// `decideJudgementCategory` の doc の「「判定が甘い」の向き」。
//
 ...
 * **甘くなる向きは「検出」側である**（#1087。根拠は `decideJudgementCategory`
 * の doc の「「判定が甘い」の向き」）。宣言が差し引く集合そのものになれば、狭く
 * 宣言した分だけ印に起因する赤が残り、下の「偽の『検出』」がそのまま出る。
