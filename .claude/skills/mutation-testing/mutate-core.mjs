 ...
 *
 * **⚠️ ただし「厳しくする側にしか動かない」が成り立つ基準を書いておくこと（#1137）。**
 * 成り立つのは **#993 以前の件数ベースの判定を基準にしたとき**である（旧: 赤が1本でも
 * あれば `検出` / 新: 宣言との交差が要る ⟹ 新 ⊆ 旧）。**「正直に狙いだけを宣言した
 * 場合」を基準にすると成り立たない** —— 実測（2026-09-16）で、走行の中身が完全に同一
 * のまま、狙い1本だけの宣言では `身代わり`、証跡の `[残った]` をそのまま写した宣言
 * では `検出` になった。**宣言を広げるほど `検出` になりやすい向きには、門が1つも
 * 無い。** 詳細と直す方向は #1137。
 *
