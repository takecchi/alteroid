...
- **`success` や `mergeStateStatus: CLEAN` が「在る」ことは、それだけでは緑の証拠にならない。正しい sha の `check-runs` を引いた後でも、3つの形で化ける**（2026-09-15 観測、#983。関連 PR #969・#975・#979・#980）。**根はどれも同じ — `CLEAN` や `success` が「在る」ことを緑と読むことである。** 直上の「これで『CI が green』の罠は3つになり」は**どの sha を見ているか**の話だったが、こちらは**正しい sha を指定した後でも**起きる。
  ...
  1. **draft PR の `skipped` を緑と数えない。** draft のあいだ `ci` / `image` は（2026-09-24 に廃止した `base-overlap` も当時は同じだった。下の生出力に出てくるのはその頃のもの） `if:` の条件で `skipped` になり `completed` を返す。`ready_for_review` 後に本物の run が走ると、**同じ sha の `check-runs` に新旧2世代が並ぶ**（PR #969・#975 で観測）:
  ...
  3. **⭐ `gh pr ready` が run を起こさないことがあり、そのとき GitHub は `mergeStateStatus: CLEAN` と言う（いちばん危ない）。** PR #980 で観測: push（`synchronize`。この時点ではまだ draft）に続けて同じコマンドの中で `gh pr ready` を打ったところ、`ready` は run を1本も作らず、`ci` / `image` / `base-overlap` が draft 由来の `skipped` のまま `mergeStateStatus: CLEAN` になった。**機構は分かっている** — required contexts（当時の `base-overlap` は意図的に外してあった。2026-09-24 に廃止。⚠️ **`contexts` の中身をここに数え上げない —— 増えるたびに腐る。** ここには「`ci` / `image` のみ」と書いてあったが、実測 2026-09-17T02:12Z で `["ci","image","no-attribution-trailers","pr-title-type"]` の4本だった。**その後 `pr-title-type` は 2026-09-22 に takecchi の判断で廃止され、実測 2026-09-22T19:14:26Z で `["ci","image","no-attribution-trailers"]` の3本に戻っている——ここでも本数を数え上げて安心しないこと、増減はまた起こる。** **持ち主は `.github/required-status-checks.json` と `pnpm check:required-status-checks`（protection と突き合わせる）である**）について、GitHub は **`skipped` を「満たした」として扱う**（当時 `base-overlap` ジョブの doc に逐語で在った。`git show b12852f:.github/workflows/ci.yml` で読める）。**draft の間だけ節約する `if:` の設計そのものが、`ready` が run を起こし損ねた瞬間に「required が全部 skipped のまま CLEAN」という牙になる。** `.github/workflows/ci.yml` の `pull_request.types` が `ready_for_review` を明示で足しているのはまさにこの事故を防ぐためだが、**それでも push と `ready` が近すぎると取りこぼされる。**
...
- **`scripts/check-pr-green.mjs` は非 success を1本の「NG」に丸めない —— `red` / `cancelled` / `out-of-scope` / `skipped` の4値に分ける**（Issue #1197）。きっかけは、PR #1193 をマージした担当が**マージ直後の `main`（push の run）**を検算して `NG` を見たとき「マージで `main` を壊した」と読みかけたこと。実際には `ci` / `image` は `success` で、`base-overlap`（`pull_request` 専用ジョブ。上の「`base-overlap` を required contexts には入れていない」の逐語のとおり push では走らない）が設計どおり `skipped` になっていただけだった。**`conclusion !== 'success'` の1本判定では、この `skipped` も本物の `failure` も同じ「NG」に見える。**

  | verdict                                | 意味                                                                                                                                                                                                                                                                             | 終了コード                                                                                                                                |
  | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
  | `green`                                | 最新世代の job がすべて `success`                                                                                                                                                                                                                                                | 0                                                                                                                                         |
  | `red`                                  | `cancelled` でも `skipped` でもない非 success（`failure` 等）が1本以上                                                                                                                                                                                                           | 1                                                                                                                                         |
  | `cancelled`                            | red は無いが `cancelled` が1本以上 —— **赤ではない。走り切っていない**（原因を取り違えないよう `red` と文言を分ける。#1155 の積み残し）                                                                                                                                          | 1                                                                                                                                         |
  | `out-of-scope`                         | red も cancelled も無く `skipped` が在り、**最新 run のどれも `pull_request` イベントではない**（`event=push` 等） —— この道具は PR 用であり、対象 sha は対象外。`pull_request` 専用 job の skip は設計どおりで、**skip 以外の job はすべて success だった**ことも同じ出力で言う | **2**（0でも1でもない第3の値。呼び出し側が「1=NG=壊れている」と早合点する事故——まさに#1197——を、他のNG系と同じ1に丸めると再発させるため） |
  | `skipped`                              | red も cancelled も無く `skipped` が在り、最新 run に `pull_request` のものが在る（または `event` が読めず判定できない——安全側） —— **draft 由来の skip の疑いを含むので緑にしない**                                                                                             | 1                                                                                                                                         |
  | `pending` / `unmeasurable` / `no-runs` | 従来どおり                                                                                                                                                                                                                                                                       | 1                                                                                                                                         |

  **安全側の詰め:** `skipped` 以外の job が0本（実行された job が全部 skip）のときは `out-of-scope` と名乗らず `unmeasurable` へ落とす —— 「skip 以外はすべて success」が空虚な主張になるのを防ぐ（schedule の run で `ci` が丸ごと skip される標本での偽の安心を防ぐ詰め）。detail には**どの verdict でも非 success の job を全部**（名前・workflow・run id・conclusion）並べ、`out-of-scope` や `cancelled` が本物の赤を隠さないようにしてある。**世代選び（`pickLatestRunPerWorkflow` の `created_at` / `id` tiebreak）は1文字も変えていない。** 実装は `scripts/check-pr-green-core.mjs` の `evaluatePrGreen`。
...
- **draft の run が `conclusion: success` を名乗るようになった —— draft でも走る門が1本増えたせいで、run 全体の結論が緑に見える。** `.github/workflows/ci.yml` の `no-attribution-trailers` ジョブは、`ci` / `image` / `base-overlap` と違って draft の間も skip しない（`if: github.event_name == 'pull_request'` のみで、他の3ジョブが持つ draft 判定を持たない）。⟹ **この1本が `success` を返すだけで、`ci` / `image` / `base-overlap` が全部 `skipped` のままでも run 全体の `conclusion` は `success` になる。** 実測（2026-09-15 観測、PR #1021 の draft 中の run `34970519704`。自分で取り直した）:
  ...
  **⟹ 変わったのは結論ではなく、見るべき run の本数である。** 同じ draft の PR（head `2f50b763`）で run は3本に分かれ、**`CI` 以外の2本は draft でも `success` を返す**（同じ観測）:
  ...
  この2本が draft でも skip しないのは**意図である**（逐語は `grep -Fn -- 'draft のあいだも毎回走らせ' .github/workflows/no-attribution-trailers.yml` 。当時は `pr-vanished-footprint.yml` もそうだったが、2026-09-24 に廃止した）。⟹ ⭐ **「緑の run が在る」は `ci` が走ったことを意味しない、はそのまま効く。** 化け方が「1本の run の中で `conclusion` が緑になる」から「**緑の run が別に在る**」へ移っただけである。**ジョブの内訳まで降りろ**も変わらない —— `scripts/check-pr-green.mjs` は workflow 名ごとに最新の run を選ぶので、3本に分かれても正しく答える（#1108 で実測）。
