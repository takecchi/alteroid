...
- `cp .env.example .env` → `docker compose up -d` → `docker compose exec app alteroid token add --label <名前> -f <path>` → `docker compose exec app alteroid chat`。埋めるのは2つ（`ALTEROID_RUNNER_TOKEN` / `ALTEROID_DATABASE_URL`）。**`CLAUDE_CODE_OAUTH_TOKEN` は2026-09-14 から環境変数では持たない**——認証トークンのプール（DB 正本）へ `alteroid token add` で登録する。ホストのディレクトリを workspace にするなら先に `mkdir -p workspace`（Docker に作らせると root 所有になり、コンテナ内の `node` が書けない＝「コンテナだからできない」が生まれる）
  ...
  - マネージャーへ渡す MCP 設定・プロジェクト設定は `workspace/`（＝runner コンテナの `/workspace`）に置く。cwd がそこなので `settingSources` の `'project'` / `'local'` が cwd から拾う（値そのものは `claude-provider.ts` の `buildManagerSessionOptions` が持つ）
  ...
  - **⭐ runner は自分の環境変数から鍵を1文字も拾わない**（人間の決定 2026-09-11。`apps/runner/src/index.ts` の `seed: {}` と `runner.ts` の `#childEnv()`）。器に置いてもマネージャーには渡らない ⟹ **runner 側の `GH_TOKEN` / `GIT_*` / `CLAUDE_CODE_OAUTH_TOKEN` は消してよい。消すのが正しい**
  - **配る値の出所は「正本 → 無ければクローンの器の env」。** どちらにも無ければ配らず、マネージャーは資格を持たずに走る（その事実は出力に残る）。認証トークンだけは回し手が撒く（`token-spread.ts`。env の行もデーモンの env の値を持って降りる）
  - **`CLAUDE_CODE_OAUTH_TOKEN` は置けない**（400）。正本はプールの側（`alteroid token`）で、こちらへ置くと名乗り直しのたびに回した鍵を巻き戻す（`.claude/skills/token-pool/`）
  - **プロファイル（`alteroid profile`）との使い分けは「走行中に届くか」である。** こちらは名前ごとにファイルへ落ちるので `gh` / `git` は次の呼び出しから読み直す。あちらは SDK 子プロセスの起動時にしか効かない。そして `GET /profile` は本文ごと返すが、こちらは指紋しか返さない ⟹ **秘密はこちら側へ置く**
- マネージャーに PR を出させるなら `.env` に `GH_TOKEN` と `GIT_AUTHOR_*` / `GIT_COMMITTER_*` を足す形も引き続き効く（**app に在れば足りる。**そこが最後の土台になる）。無くても公開リポジトリの clone は通る。手順とスコープは [railway/README.md](./railway/README.md) の「マネージャーに GitHub を渡す」。**`gh` の版は固定していない** — 固定すると人間の手元より古い `gh` を配ることになり、その遅れがデグレードになる
