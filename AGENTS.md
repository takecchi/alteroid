...
- **閉じるキーワード（`close` / `fix` / `resolve` の活用形 + `#番号`）を、意図して閉じるとき以外は PR 本文にもコミットメッセージにも書かない。参照するだけなら番号だけを置く**（#1109）。閉じたいときは**キーワードと参照だけで構成された行**にする（または `gh issue close <N>` で手で閉じる）
  - **⚠️ 散文で「閉じる」と書くだけでは Issue は閉じない**（GitHub のパーサは日本語の閉じる意思を読まない。実例は直下の「PR #1073」）。**閉じたいなら PR 本文に独立した行で `Alteroid-Issue-Done: <番号>` を書く**（正本は `scripts/issue-done-trailer-core.mjs`。マージ後に workflow が閉じる）。閉じたくないなら `Alteroid-Issue-Done: none` で降りられる（#1134）
...
- **Claude 本体（`@anthropic-ai/claude-agent-sdk`）の版を手で上げに行かないこと。** `.github/workflows/update-claude-sdk.yml` が毎日見て、上がっていれば1本の PR（`automation/claude-agent-sdk`）に積み直す。**その PR は AI の層が自分の判断でマージしてよい**（2026-09-16 の人間の決定。#1053 — ここには「マージするのは人間である」と書いてあり、PR #1044 を AI の層がマージしたことをきっかけに人間へ諮って置き換えた）
  ...
  - **⛔ squash するときは本文を明示すること（`gh pr merge <N> --squash --body "<本文>"`）。既定の本文でマージしないこと**（#1350 の決定。選択肢 C）。この枝のコミットは `github-actions[bot]` の名義なので、GitHub は既定の squash 本文の末尾へ bot を共著者とする行を足し、それが `main` へ焼かれて `main のトレーラを知らせる` が赤くなる（実例: #1430 → `045968c` → #1432）。**本文を明示すれば、GitHub は行を足さないはずである** —— 足されるのは既定の本文を組み立てるときだけ、という GitHub の挙動に拠っている。**⚠️ この枝の PR で実測はしていない**（決めた時点で開いている更新 PR が無かった）⟹ **この形で最初にマージした人は、`git log -1 --format=%B <マージコミット>` で行が無いことを確かめて #1350 に書くこと**。⟹ マージの口で1回渡せば済み、枝の名義（A）やマージの方式（B）を変えずに止まる。**検知の門は外さない** —— 本文を渡し忘れた回を拾うのは、いまもあちらである
