...
- **main へのマージはデプロイの引き金である。** 器が入れ替わると、その中で手を動かしていたマネージャーと作業者は畳まれる（2026-08-14 に走行中だった4本が失われている）。落ちる Service は `railway/daemon.json` / `railway/runner.json` の `watchPatterns` で決まり、`docs/` や `.gitignore` だけの PR ではどちらも落ちない。範囲の内訳と、被害を減らすために置いてある仕組みは [railway/README.md](./railway/README.md)「デプロイは走行中の仕事を畳む操作である」
