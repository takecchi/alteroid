...
- 構成は **daemon（クローン＋記憶）/ manager-runner（SDK）/ PostgreSQL** の3つ。マネージャーは runner の中だけで走る
  ...
  - runner を立てていないローカルでは同一プロセスの runner に落ちる（宛先の環境変数が無いとき）。**そのときは既知の穴が残る** — 塞ぐのはコンテナ構成の役目で、ツール削除ではない
  - 委譲の宛先は `RunnerRegistry` 越しに決める。固定 URL も runner のローカルパスもデーモンに書かない
  - **「器へ届かない」を「器が止まった」と読み替えないこと。** ネットワークだけが切れて中のマネージャーが走り続けている状態と、コンテナが消えた状態は、デーモンからは区別が付かない。そこで同じ session を別の器で開くと**1つの仕事が2か所で走る**（同じ workspace への二重書き、`gh pr create` や Slack 送信の二重実行）。デーモン側の排他は別プロセスの1本を止められない
    - 根拠になるのは器の側の**貸し出し期限**（`ALTEROID_RUNNER_LEASE_TTL`、秒。既定 30）。この時間 `GET /health` が届かなければ runner が自分でセッションを畳み、デーモンは最後の名乗り＋期限＋**器が申告した猶予**（`ALTEROID_RUNNER_LEASE_GRACE`）＋余裕を過ぎてから移す（`apps/runner/src/lease.ts` と `packages/core/src/manager.ts` の `#fence`）
    - **器が畳み始める時刻を、デーモンの計算より後ろへずらさないこと。** 期限の見張りを一定間隔で回すと、最後の名乗りとの位相のぶん畳み始めが遅れ、そこが二重に走る窓になる（見張りは名乗りのたびに張り直す）
    - **申告した猶予は守らせる。** 畳み終わらなければ器ごと降りる（`onGraceExceeded`）。デーモンは時刻が来ただけで移送するので、「まだ畳めていません」と粘るのがいちばん危ない。分断されている以上、畳めなかったことを伝える経路も無い
    - **停止確認は「届いた」ではなく「畳んだ」で取る。** `stop` は在ったかどうかを返す（`RunnerClient.stop`）。`runner_id` は器を作り直しても同じ名前で戻るので、新しい器の「知らないので何もしなかった」を確認と読むと、古い器と二重に走る
    - 同じ理由で、**どの起動に置いたか**（`lease.incarnation` / `job.runnerIncarnation`）まで台帳に残す。名前だけで宛先を決めると、入れ替わった器へ「続き」を流す
    - **期限の計算には、その起動について観測したものだけを使う**（最後に見た時刻も、名乗っていた ttl / grace も）。いまの器の値を流用すると、旧器が長い期限、新器が短い期限、というローリング更新で早すぎる移送になる。観測が無ければ移さない（時間だけで安全と決めない）
    - **期限を延ばす口を `GET /health` 以外に増やさないこと。** 増やすと器の期限がデーモンの見立てより後ろへずれ、走っている仕事を移すことになる
    - 期限を報告しない器の仕事は自動では移らない。**それはバグではなく安全側である** — 確かめた側が `manager_move`（chat では `/move <id> force`）で引き取る
  - **runner を待ち合わせる呼び出しには必ず期限を置く**（`withDeadline`）。接続を拒む相手なら例外はすぐ返るが、TCP が繋がったまま黙る相手は期限が無ければ永久に解けない。生存判定も配置も全 runner を待ち合わせるので、1台の沈黙で名簿ごと止まる＝「1台の不在が残りを止めない」（M5 受け入れ基準5）が崩れる
...
- 宛先は `ALTEROID_RUNNER_URLS`（カンマ区切り）。`ALTEROID_RUNNER_URL`（1台）も読むので、両方あれば合わせて名簿に載る。**器ごとに `ALTEROID_RUNNER_ID` を変えること** — 台帳の `manager_id → runner_id` はこれで引くので、被らせると宛先が壊れる
  - 合鍵（`ALTEROID_RUNNER_TOKEN_SHA256`）は**全 runner で同じ**。制御面の本人確認は「デーモンかどうか」の判定であって、器を区別するためのものではない
  - **1台構成では、名乗りが返らなければ起動しない**（宛先の無いデーモンは何もできない）。**複数構成では残りで走り**、返らない器は名簿に載ったまま生存確認の対象になる（1台の不在で全部を止めない）
  - ローカルで2台を試すなら `docker compose --profile scale up -d` ＋ `.env` に `ALTEROID_RUNNER_URLS=unix:/run/alteroid/runner.sock,unix:/run/alteroid/runner-b.sock`。Railway は [railway/README.md](./railway/README.md) の「runner を増やす」
- 配置は runner が `/health` で報告する**実行環境の資源**（CPU・メモリ・稼働セッション数）で決める。`packages/core/src/runner-registry.ts`
  - **定員を作らないこと。** 「同時に何本まで」は能力の削除であって配置の判断ではない（禁止2 / roadmap M5 の地雷）。`select` は詰まっていても必ず置き先を返し、断るのは1台も登録されていないときだけである
  - コンテナでは `os.totalmem()` がホストの量を返すので、cgroup v2 を優先して測っている（`capacity.ts`）。ここをホスト値に戻すと、512MB の器も 64GB の器も「余裕あり」に見えて配置が実態から外れる
  - 生存判定は heartbeat（既定 10 秒間隔 / 30 秒で不通と見る）。**前回の判定結果と突き合わせている** — 経過時間だけで見ると落ちた瞬間が誰にも観測されず、フェイルオーバーが起きない
- 器が落ちた委譲を別の器へ移せるかは、**workspace の運用選択**（`ALTEROID_WORKSPACE_KIND`）で決まる
  | 値                                                | 別の器で続けられるか                                                                               |
  | ------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
  | `runner-volume`（既定）                           | 不可。セッションは預かってあるが、未コミットの作業は復旧できない                                   |
  | `shared-volume`（`ALTEROID_WORKSPACE_PATH`）      | 可。全 runner が同じパスで同じ FS を見ている前提                                                   |
  | `git`（`ALTEROID_WORKSPACE_REPOSITORY` / `_REF`） | 可。**マネージャー自身に clone し直させる**（デーモンが器の中で clone を代行する仕組みを足さない） |
  - **移せないときに黙らないこと。** 何が残り（セッション）何が失われたか（未コミットの作業）をクローンの受信箱へ上げる。判断はクローンがして、記憶に根拠が無ければ人間へ回る（roadmap M5 受け入れ基準4 の後段）
  - 移送したふりもしない。作業ディレクトリが無いのに「続けられる」と伝えると、マネージャーは空の木の上で作業を書き始める
- 何台居て・どれが生きていて・どれだけ余裕があるかは `GET /runners`、chat では `/runners`。どのマネージャーがどの器に居るかは `/managers`（移送先もここに出る）
- 複数構成の検証は SDK 抜きで回る。器ごとに偽の `query` を渡して「どちらの器で走ったか」を見る（`packages/core/src/manager-multi-runner.test.ts`）。1台構成と3台構成で能力・プロトコルが同じであることの回帰もここにある
