...
- **`find` も同じで、この器では `bfs` が走る ⟹ `-newermt "@<秒>.<小数>"`（`date +%s.%N` の値）は黙って0件になる。** Claude Code が profile で `find` という bash 関数を注入していて、実体は `claude` バイナリを `bfs` として起こしたもの（`type find` で見える）。`bfs` は小数点以下の付いた epoch を `Invalid timestamp.` として拒み、stderr に出して exit 1 で終わる。**`| wc -l` の後ろに置くと、`0` と exit 0 だけが返る**（上の「パイプの終了コード」と同じ族）。実測（2026-09-24、`bfs 4.1.1`）: 同じ窓で `-newermt "@$(date +%s)"`（整数秒）は1件、`-newermt "@$(date +%s.%N)"` は0件。`command find`（GNU findutils 4.10.0）は小数秒でも1件を返す。⟹ **整数秒の `date +%s` を渡すか、`command find` を使う**（#1437 の歯を数えたときに踏んだ）
