...
| 用途が増えるたびに `compose.yaml` へ環境変数を足す            | 人間が `.zshenv` に1行足すだけの作業が実装作業になる（デグレード）   | **名前と値**なら `alteroid credential set <名前>`、**スクリプト**が要るなら `alteroid profile edit`。器を焼き直さない（⚠️ 鍵をプロファイルへ書くと鍵の口を影にして、回した鍵が黙って巻き戻る） |
