...
**枝を消す前の「着地したか」の判定は、`compare` の `behind` / `identical` では取れない。squash マージに効かないためである。⟹ 使う手順は4段で、1段でも取れなかったら消さない。** ⚠️ **「判定できない」を「消してよい」へ倒さないこと。** **4段の手順そのもの・2段目が要る理由の実測・`headRefOid` と先端が食い違うもう1つの理由（同名の枝の作り直し）・ref がいつ動いたかを `committer.date` で測らないことは、`.claude/skills/branch-cleanup/SKILL.md` に在る**
...
- **最初の報告に、この文書の状態を1行で書く。行為ではなく状態を書く** — (1) 作業ツリーの直下に在るか（`ls` で分かる） (2) 自分の文脈に載っているか、**セッション開始時と、いま**（自分の文脈を見れば分かる）。**どちらも、見れば答えられる。**
  ...
  - **状態にすると、答えるために見に行くことになる。それ自体が観測である。** そして**2026-08-22 の4例は全部この2つで表せる** — 開始時に無く後から載った／最初から載っていた／別のツリーでだけ載った／最後まで載らなかった
...
## CI の完了を待つ形（`gh run watch` を背景に置かない）

**待ち方を間違えると、待ちが終わらないのではなく「委譲そのものが止まる」。** 2026-09-17 に作業者2人が同じ形で止まった。

```
worker a25a28c41 → gh run watch 35196482974 --repo takecchi/alteroid --exit-status 2>&1 | tail -60
worker a86fd5bd3 → gh run watch 35195863856 --repo takecchi/alteroid --exit-status 2>&1 | tail -40
```

⛔ **CI の完了待ちに `gh run watch` を背景処理で使わないこと。** 3つが同時に効く:
...
✅ **代わりに、上限付きのポーリングで待ち自体が必ず終わる形にする。** `gh api repos/takecchi/alteroid/commits/<head_sha>/check-runs` を**回数の上限を先に決めて**叩く（例: 20回 × 30秒 = 最大10分で必ず抜ける）。⭐ **head sha を明示すること** — PR 番号だけで引くと、draft 中の `ci.yml` が返す `conclusion: skipped` を緑と読む。**結末は独立した2つの出所で確かめる**（`check-runs` API と `gh run list --commit <sha>`）。

⭐ **止まった作業者は、残った背景処理を片付けると自力で再開する。** 実測（2026-09-17）: 上の `gh run watch` のプロセスを落としたところ、**2人とも数分内に `running` へ戻り、片方はその後に最終報告まで出した。** ⟹ **「作業者が死んだので引き取る」と決める前に、まず残った背景処理を見ること。**

⛔ **ただし、引き取ると決めたなら作業者のツリーには触らないこと。** 同じ日に、止まったと判断して作業者のツリーで変異を当てたところ、**その作業者は再稼働していて同じファイルを同時に書いていた** — 相手の変異試験の赤/緑がこちらの変異と混ざり、測り直しになった。**プロセス一覧の見え方で生死を決めないこと**（「自分が走っている器」の同じ注意）。
...
- **検証コマンドを漏れなく渡す**（正本は `ci.yml` の `ci` job の `run:` と `scripts/verify-core.mjs` の `STEPS`。ここに写さない — 写した列挙は腐る。`check:*` の配線は `scripts/check-scripts-wired.test.ts` が突き合わせる）。**渡さなかった検証は実行されない** — `typecheck` を渡し忘れて型エラー2件が CI まで残った
...
- **`Co-Authored-By:` / `🤖 Generated with [Claude Code]` のトレーラ・フッタを付けない**（コミットメッセージにも PR 本文にも）。**規約は上の1行 `<type>: <description>` だけで、トレーラを含まない**
  ...
  - **決定は両方の印に掛かる。**クローンが `Co-Authored-By:` と `🤖 Generated with [Claude Code]` の**両方を名指しした**問いに、takecchi は「つけない」と答えている（2026-09-15）。実測（2026-09-15T12:14Z 観測）: `main` 740本中 84本が `🤖 Generated with` を持つ（2026-09-15 の1日だけで6本）。CI の門は Issue #1020 / `scripts/check-no-attribution-trailers-core.mjs`
...
- **閉じるキーワード（`close` / `fix` / `resolve` の活用形 + `#番号`）を、意図して閉じるとき以外は PR 本文にもコミットメッセージにも書かない。参照するだけなら番号だけを置く**（#1109）。閉じたいときは**キーワードと参照だけで構成された行**にする（または `gh issue close <N>` で手で閉じる）
  - **⚠️ 散文で「閉じる」と書くだけでは Issue は閉じない**（GitHub のパーサは日本語の閉じる意思を読まない。実例は PR #1073 —— 本文の「#1072 を閉じる。」は不活性で、#1072 は人間が手で閉じるまで開いていた（`scripts/issue-done-trailer-core.mjs` の冒頭））。**閉じたいなら PR 本文に独立した行で `Alteroid-Issue-Done: <番号>` を書く**（正本は `scripts/issue-done-trailer-core.mjs`。マージ後に workflow が閉じる）。閉じたくないなら `Alteroid-Issue-Done: none` で降りられる（#1134）
  ...
  - **CI の門（`pr-closing-keywords`）は 2026-09-24 に takecchi の判断で廃止した**（逐語「テストやビルド以外の無駄なワークフロー消してほしい」）。**⟹ 規約は生きている。機械は見ていないので、書く人が確かめること。** 判定の関数（`scripts/check-pr-closing-keywords-core.mjs`）は `check-keyword-closed-issues` / `issue-intent-hint` / `issue-done-trailer` が使っているので残してある。workflow 本体を読むなら `git show b12852f:.github/workflows/pr-closing-keywords.yml`
...
- **git / GitHub の actor から層を推定しないこと。** クローンもマネージャーも人間の `GH_TOKEN` と `GIT_AUTHOR_*` / `GIT_COMMITTER_*` をそのまま使う（配り方は `.claude/skills/cloud-deployment/` が持つ）ので、**コミットの author も PR の actor も、3層すべてで人間の名前になる。層は git の外にしか無い** — どの委譲がどの変更を作ったかは alteroid 自身の記録（クローンの日誌・委譲の台帳・ブランチ名）にしか残らない。**これは欠陥ではなく、直上のトレーラの決定の裏面である**（印を付けないと決めた以上、git 側に層は現れない）
  ...
  - **⚠️ その「もし」が実際に起きた（2026-09-17T07:20Z、PR #1167）。** マネージャーがマージした PR を、**同じ委譲の作業者が「human/`takecchi` による直接マージ」と報告した。** 取った材料は正しい —— `gh pr view --json mergedBy` が `{"login":"takecchi"}`、timeline の `merged` / `closed` の actor も `takecchi` である。**読み方だけが違った。**
    - **出発点も正しかった** —— 「自分は打っていない」から始めている。**分岐したのはその次で、`mergedBy` を根拠に「では人間が打った」へ進んだ。** ⟹ 上の PR #155 の人は同じ地点から「自動経路を疑い、範囲外として渡す」へ進んだ。**同じ材料から、逆の結論へ行ける。**
    - ⛔ **この作業者の文脈には、この節を含む本文が全文載っていた**（自己申告と、載った時機の報告が在る）。⟹ **書いてあっても踏む。**「規約に書くことは必要条件であって十分条件ではない」（#1046）と同じ形である
    - **観測できたのは、作業者が「自分はマージしていない」を報告に書いたからである。** 書かなければ、誤った帰属がどこにも現れないまま残った
...
- **sha を出典として指すときは、消えうる側ではなく残る側を指す** — 畳む側の PR の sha は rebase で書き換わるので、そこを指すと出所ごと消える。⟹ **`main` に在る sha を指すこと**（枝の sha しか無いなら `.claude/skills/branch-cleanup/SKILL.md` の「消えてしまった枝から中身を取り戻す」の形で残す）
...
- **この文書が自分の文脈に載っているとは限らない。** セッションの `cwd` は `/workspace` で、この文書は `/workspace/<自分のID>/repo/` の中にあるので、**開始時には載っていない。** 後から載ることがあるが、**何が載せるのかは分かっていない**（観測例の内訳は上の「書く先を決める」に在る）。**「読んだつもり」で判断しないこと** — 載っていなければ `cat` でも `Read` でも自分で読めばよく、**読んだかどうかを報告に書けば、依頼者はどこまで信じてよいかの線を引ける**
...
- **`python3` は無く、`pkill` / `pgrep` も実体が無い。** 作業用のスクリプトは node で書く
  ...
  - **そもそも掃除が要るのは、`Bash` が制御を返した（`timed out` / `moved to the background` など。文言は版で違う。「踏みやすい地雷」の 120 秒の項）ことを実プロセスの終了と読んだときである。** あれはツールが待つのをやめた合図であって、子プロセスは走り続けている。**起こす時点で `timeout <秒> …` に寿命を委ねれば、掃除そのものが要らない**（`/usr/bin/timeout` は在る）
...
- 検証の一式と順序の正本は `ci.yml` の `ci` job の `run:`（写さない。上の「検証コマンドを漏れなく渡す」と同じ）
