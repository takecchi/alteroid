...
- **検証コマンドを漏れなく列挙して渡す**（`build` / `check:web-bundle-node-traces` / `check:web-bundle-size` / `apps/daemon/openapi.json` に差分が無いこと / `check:sdk-quotes` / `typecheck` / `lint` / `format:check` / `test`）。**渡さなかった検証は実行されない** — `typecheck` を渡し忘れて型エラー2件が CI まで残った
...
- `pnpm install` → `pnpm build` → `pnpm check:web-bundle-node-traces` / `pnpm check:web-bundle-size` / `apps/daemon/openapi.json` に差分が無いこと / `pnpm check:sdk-quotes` / `pnpm typecheck` / `pnpm lint` / `pnpm format:check` / `pnpm test`
