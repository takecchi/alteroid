...
**そして上げた後にどちらへ倒すかは、「誰が作った穴か」で切る。** 上げたものをそのまま同じ PR で直したくなる — その線引きである。
...
実例(2026-08-23、#285): 画面の分岐を `else` で受けていて、**将来 enum に足された値が黙って網の外へ出る**形になっていた。これは**その PR が作った穴なので同じ PR で塞いだ**（`switch` ＋ 型の網羅性）。同じ画面の別の箇所も未知の値でラベルが空になるが、**そちらは以前から在る挙動なので Issue（#288）にした。**
