...
- 外部サービス接続は **MCP**
- 能力の口は **HTTP API** である。CLI（`alteroid`）も WebUI（`apps/web`）も**その上に載る器**であって、自分で機能を持たない（マルチユーザーは非ゴール）
  - WebUI はデーモンの API を同一オリジンで前へ通すだけである。**画面のために API を分岐させないこと** — 分岐させた瞬間、CLI と画面のどちらかが遅れて能力の差になる（north_star 禁止1）。増やすなら API に増やす
  - **プロキシで `content-type` を補わないこと。** デーモンが本文の無い POST に `application/json` を要求しているのは、ブラウザの単純リクエストで他人がクローンのターンを起こせないようにする境界である（`deliberateClient`）。気を利かせて付け足すと、その境界がプロキシの内側で消える
...
- ブラウザから使うなら `alteroid web`（既定 `http://127.0.0.1:4518`。`ALTEROID_WEB_PORT` / `ALTEROID_WEB_BIND`、`--port` / `--bind`）。chat と同じくデーモンが居なければ起こす
  - デーモンが別の器で既に走っているなら `alteroid-web` 単体でもよい（接続先は `ALTEROID_DAEMON_URL`、無ければ `http://127.0.0.1:$ALTEROID_PORT`）
  - **画面はデーモンと同一オリジンで配信する**（`/api/*` がプロキシ）。デーモンは CORS ヘッダを返さない＝別オリジンの画面からは読めない。ここを CORS で開けて回るより、器を同じオリジンに置くほうが境界の形を変えずに済む
  - デーモンと同じく認証は無く、既定は 127.0.0.1 で待つ。外へ出すなら手前に境界を置くのが先
