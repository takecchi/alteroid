...
- **`Bash` の出力は 30,000 文字あたりで切られ、打った側に届くのは先頭 2KB のプレビューだけである。⟹ `cat AGENTS.md` はこの文書の冒頭しか届けない。** 実測(2026-08-27): `cat AGENTS.md`（122,432 バイト）の戻りは `Output too large (119.6KB). Full output saved to: <パス>` で、本文として届いたのは「まず読む」「この文書の役割」と「書く先を決める」の冒頭だけだった。閾値は `head -c <N> /dev/zero | tr '\0' 'a'` で挟んだ — **29,500 文字は素通りし、30,100 文字は落ちた**（後者の表示は `Output too large (29.4KB)`）。**⚠️ これは失敗として返らない。** 出力は在り、exit code は 0 で、打った側には「読んだ」に見える。**⟹ `Bash` でこの文書を読むなら自分で窓を切ること**（`awk 'NR>=A && NR<=B' AGENTS.md`）。**そして1行が長いので、行数で切っても 30,000 文字を超えうる** — 実際に `sed -n` で 230 行ぶんを一度に出して落ちた（`Output too large (45.2KB)`）
