...
- **`/tmp` も同じ器の中で共有される。一時ファイルは `/tmp/<自分のID>/` の下へ置く。** 実測（2026-08-22、runner-3）: あるマネージャーが `/tmp/mutate.mjs` を書こうとしたら、**同名で別のマネージャーのもの**（別対象の変異試験ハーネス）が既に在った。書き込みツールの「未読ファイルは上書きさせない」で止まり、潰さずに済んだ。2人とも実際に `/tmp/<自分のID>/` へ退避した。**確認できたのは同じ器（コンテナ）の中で共有されることまでで、runner をまたいで共有されるかは確認していない**（別 Railway Service なので別コンテナのはずだが、未確認）
