...
- **他のファイルを出典として指すときは、行番号を単独の出典にしない。逐語の一部かシンボル名で指す**（`grep -n '<逐語>' <path>` / `<path>` の `<関数名>`）。**この文書・コードの注釈・PR 本文のどれでも同じである**
  ...
  - **歯が在る**（`scripts/agents-md-references.test.ts`）。**測っているのは参照の形だけで、内容は測っていない** — (1) リポジトリ内のファイルを `path:行番号` で指していないか (2)「N行目」で指していないか (3) `grep -n '<逐語>' <path>` の形で書いた出典が現物に当たるか。**フェンスの中は見ない**（直上の証拠の側）。**(3) が落ちたときは「行が動いた」ではない** — 歯は行番号を一切見ていないので、落ちたなら指した文言そのものが変わっている
...
- **同じ形の顔が4つある。** どれも「同じツリーで build が重なった」の別の出方でありうるが、**同型だと確かめてはいない**
  ...
  4. **`SIGABRT`（exit 134）** — **⚠️ これは今回の実測ではなく既存の記録である。** `scripts/verify.mjs` の `run(step)` の doc に在る（逐語は `grep -n '直接打つと通るのに、この口から呼ぶと落ちる' scripts/verify.mjs`）
