...
- **いつ載るかの機構は特定されていない。** 実測が3つあり、**3つとも形が違う**（すべて 2026-08-22、別々のセッション）
...
- **だから「`Read` が引き金」とは書けない。** 3例目がそれを否定する。**言えるのは「開始時に載っていないことがあり、後から載ることがある。何が載せるのかは分かっていない」までである。** これ以上を書くと、この文書はまた自分について嘘をつく
