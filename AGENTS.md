...
- **時刻を assert するテストは、器の時間帯に任せず自分で `TZ` を固定する。上の9時間差がそのままテストに出る** — 手元の器は `TZ=Asia/Tokyo` だが **CI の runner は UTC** である（`.github/workflows/ci.yml` に `TZ` の指定は無い）ので、期待値が**手元だけ緑**になる。**期待値を器へ合わせて書き直すと、こんどはもう片方の器で落ちる。直し方は「注入する側を自分で用意する」であって、期待値を器へ寄せることではない。表示側は固定しない**（人間は JST で読むので、時間帯は閲覧者の端末に委ねるのが正しい）。実装済みの形と、`vi.hoisted` でなければ静かに効かない理由が `apps/web/app/routes/reports.test.tsx` の冒頭にある
...
- **`pnpm test` は器が混んでいると、既定の並列度で「テスト0本のまま `exit 1`」になる。** vitest の fork pool が `write EPIPE` で死ぬ形で、**出力は30行未満で終わり `Test Files` / `Tests` の行が出ない。** `exit 1` だけを見ると「テストが落ちた」と読めるので、**存在しない失敗を直しに行くことになる**
  ...
  - **対処は並列度を下げること。** `pnpm test --maxWorkers=4` で 86 files / 1,379 tests が完走した（vitest は既定で `nproc` に応じた数の fork を起こす）。**器の CPU 数をこの文書に書かない。要るなら `nproc` で自分で取ること** — 器は焼き直され台数も変わるので、**書いた数は書いた瞬間から腐り、腐ったことは読む側からは分からない**（ここには `32` と書いてあり、別の器で `48` という報告が上がっていた）。**全スイートは回す** — 回す範囲を狭めるのは能力を削る側で、並列度を下げるのはバーを保ったまま器に合わせる側である
