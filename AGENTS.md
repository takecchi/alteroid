...
- **閉じるキーワード（`close` / `fix` / `resolve` の活用形 + `#番号`）を、意図して閉じるとき以外は PR 本文にもコミットメッセージにも書かない。参照するだけなら番号だけを置く**（#1109）。閉じたいときは**キーワードと参照だけで構成された行**にする（または `gh issue close <N>` で手で閉じる）
  - **⚠️ 散文で「閉じる」と書くだけでは Issue は閉じない**（GitHub のパーサは日本語の閉じる意思を読まない。実例は PR #1073）。**閉じたいなら PR 本文に独立した行で `Alteroid-Issue-Done: <番号>` を書く**（正本は `scripts/issue-done-trailer-core.mjs`。マージ後に workflow が閉じる）。閉じたくないなら `Alteroid-Issue-Done: none` で降りられる（#1134）
...
- **`python3` は無く、`pkill` / `pgrep` も実体が無い。** 作業用のスクリプトは node で書く
  ...
  - **そもそも掃除が要るのは、`Bash` が制御を返した（文言は版で違う）ことを実プロセスの終了と読んだときである。** あれはツールが待つのをやめた合図であって、子プロセスは走り続けている。**起こす時点で `timeout <秒> …` に寿命を委ねれば、掃除そのものが要らない**（`/usr/bin/timeout` は在る）
