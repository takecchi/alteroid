...
- **デプロイ時刻はマージ時刻ではない。** 「もう本番に入っている」という前提は `main` の履歴では確かめられない — 見るのは `gh run list --workflow=release-prod.yml` と `git log origin/release/prod` である（写しは夜2回だけ。下の「リポジトリの約束」）。実測（2026-08-20T01:44Z 観測）で JST 01:17 のマージが本番へ出たのは3時間20分後、同 06:22 と 07:47 のマージはまだ出ていない
...
- **`pnpm --filter <パッケージ> test` は1本も走らせない。** テストは root の `vitest.config.ts` 1本が集めており、`packages/*` / `apps/*` の `package.json` に `test` スクリプトは1つも無い。**pnpm は無い script を黙って飛ばして exit 0 を返す**（実測 2026-08-20: 出力0行・exit 0）。**変異試験でこれを使うと、変異が当たっていても落ちない＝偽の「歯が無い」が出る。** 絞るなら root から `pnpm test <パスの一部>`（引数は vitest へそのまま渡る）
...
- **回しているあいだ、同じ作業ツリーで他の誰か（作業者を含む）にテストを走らせない。** 別 worktree を使うか順番に回す。ファイルを一時的に壊す作業なので、並行実行は必ず汚染する（実際に汚染した）。**ただし別 worktree で分かれるのはファイルだけで、プロセス空間は共有されている**（「自分が走っている器」参照）— 走らせているプロセス自体は他の作業者から見えるし、撃たれうる
