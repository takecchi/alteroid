...
describe('編集タブ', () => {
  it('textarea に本文が出る', async () => {
    ...
    fireEvent.click(await screen.findByRole('tab', { name: '編集' }));
...
describe('保存', () => {
  it('従来どおり効く — dirty で保存ボタンが押せ、PUT の本文が入力どおりで、保存後に日時が出る', async () => {
    let putBody: unknown;
    const stub = renderDetail('notes', (url, init) => {
      if (url.includes('/memory/notes') && (init?.method === undefined || init.method === 'GET')) {
        return json({ document: DOC });
      }
      if (url.includes('/memory/notes') && init?.method === 'PUT') {
        putBody = typeof init.body === 'string' ? JSON.parse(init.body) : init.body;
        return json({
    ...
    // PUT 先が /memory/notes を叩いていること。
    expect(stub.calls.some((u) => u.includes('/memory/notes'))).toBe(true);
  });
});
