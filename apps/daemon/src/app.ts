...
export function createApp(deps: AppDeps) {
  ...
  const app = new Hono()
    ...
    .get(
      ...
      describeRoute({
        tags: ['system'],
        summary: '器の生存確認（鍵は要らない）',
        description:
          'デーモンのプロセスが応答しているかだけを返す。**鍵を要求しない代わりに、' +
          '何も明かさない** — pid も記憶の置き場も含まない（それらは `/health`）。',
        responses: {
          200: {
            description: '生きている。',
            content: { 'application/json': { schema: resolver(livezResponseSchema) } },
          },
        },
      }),
      (c) => c.json({ ok: true }),
    ...
    .get(
      ...
      describeRoute({
        tags: ['system'],
        summary: '鍵が要るかどうか（鍵は要らない）',
        description:
          'この API が本人確認を要求する構成かどうかを返す。**要求の有無は隠す価値の' +
          'ある秘密ではない** — 伏せると、CLI も画面も「401 が返ってきたから鍵が要る' +
          'らしい」と推測することになる。',
        responses: {
          200: {
            description: '要否。',
            content: { 'application/json': { schema: resolver(authRequiredResponseSchema) } },
          },
        },
      }),
      async (c) => c.json({ required: auth !== undefined && (await auth.enabled()) }),
    )
    ...
    .post(
      ...
      describeRoute({
        tags: ['system'],
        summary: '画面のためのログイン（鍵は要らない）',
        description:
          '配った鍵を、この器限りの使い捨て（cookie）に引き換える。**配った鍵そのものは ' +
          'cookie に入らない。** 使い捨ては発行元の鍵に結び付いていて、その鍵が' +
          '取り下げられた時点で通らなくなる（失くした端末をデーモンを止めずに締め出せる）。',
        responses: {
          200: {
            description: '引き換えた。`Set-Cookie` に使い捨てが乗る。',
            content: { 'application/json': { schema: resolver(okResponseSchema) } },
          },
          401: {
            description: '配った鍵と一致しない。',
            content: { 'application/json': { schema: resolver(errorResponseSchema) } },
          },
          409: {
            description: '鍵が配られていない構成（ログインは要らない）。',
            content: { 'application/json': { schema: resolver(errorResponseSchema) } },
          },
          400: {
            description: '本文が不正。',
    ...
    .post(
      ...
      describeRoute({
        ...
        summary: 'この画面だけを締め出す',
        description:
          '手元の使い捨てを捨てる。**他の端末は生きたまま**である（配った鍵ごと' +
          '取り下げたいなら、鍵の一覧から消す）。本文は無い。',
        responses: {
          200: {
            description: '締め出した。',
            content: { 'application/json': { schema: resolver(okResponseSchema) } },
