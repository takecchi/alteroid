...
// ---------------------------------------------------------------------------
// /livez, /auth, /auth/login, /auth/logout
// ---------------------------------------------------------------------------

/** 器が生きているかだけ。**鍵を要求しない代わりに、何も明かさない。** */
export const livezResponseSchema = z.object({ ok: z.literal(true) });

/**
 * 鍵が要るのかどうか。
 *
 * 隠す価値のある秘密ではない。伏せると、CLI も画面も「401 が返ってきたから
 * 鍵が要るらしい」と推測することになる。
 */
export const authRequiredResponseSchema = z.object({ required: z.boolean() });
