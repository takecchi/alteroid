 ...
 * 通るかどうかについて1文字も言っていない。そこから冷却を書くと、`markTokenUnusable`
 * が `cooldownUntil` を**上書きする**（延長しない）ため**本物の期限が未来に在る鍵を
 * 早く `ready` に見せる**（#667 の候補1を採らない理由。`token-rotator.ts` の
 * `observe` の `freshness === 'stale'` の直上に全文がある）。
 *
 ...
 * | 復帰の下限は probe の周期（5分） | **成り立たない**（判定が来ない） |
 *
