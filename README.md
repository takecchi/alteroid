...
MVP は終えた（2026-09-09、人間の決定）。ライセンスは [MIT](./LICENSE)。

**マルチユーザー / チーム利用は引き続き非ゴールである**（[docs/PRD.md](./docs/PRD.md)「スコープ外（非ゴール）」）。
ライセンスを置いたのは**他人が自分の器へセルフホストできる**ようにするためであって、1つのインスタンスを
複数人で使えるようにするためではない。この2つは別の軸である。
