 ...
 * 読み手は選べない——`usage_notice`（この節を書いた時点では `manager_report`
 * の本文を読むしかなく、`manager_list` の各行にも digest にも載っていな
 * かった）を直接名指しする軸をここに足す。
 *
 * **⚠️ 上の「`manager_list` の各行にも … 載っていなかった」は、この節が
 * 書かれた時点（#1212 残件2）の状態である。** その後（#1212 残件2の続き）
 * `manager_list` の各行にも `usageStoppedLine`（`tools.ts`）が同じ注記を
 * 付けるようになった——だから下の {@link USAGE_STOPPED_NOTICE} 本文は
 * 「絞る綴りは無い」ではなく「行の注記で名指しされる」と言う。**digest は
 * 依然として載せていない**（`digest.ts` を変える理由が無いと判断した——
 * 判断の経緯は `tools.ts` の変更を運んだ PR の本文）。
 *
 * ## ⚠️ `idle` から外さない
 *
