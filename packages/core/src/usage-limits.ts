...
/**
 * **世代ずれの委譲を起こし直せ、という助言の唯一の生成元**（Issue #1175）。
 *
 * ## なぜ1箇所へ寄せたか
 *
 * この助言は 2026-09-22 時点で**6箇所**に散っていた——`describeTokenGeneration`
 * （提案1）／`describeTokenGenerationUnknownReason` の `reattached-across-restart`
 * ／`describeResetTimeSkew`（提案(2)）／`manager_list` と `runner_list` の道具の
 * 説明文／`ManagerSummary` 側の型の doc。**#1175 の本文は「3箇所」、そのコメントは
 * 「4箇所目が在る」と言っていたが、どちらも過小だった。**
 *
 * そして**守りが揃っていなかった**: #914 の 2026-09-15T21:01:54Z が名指しした危険
 * （逐語「**失われるものを過小に言っている**」）に答えていたのは、6箇所のうち
 * `describeResetTimeSkew` の1箇所だけである（PR #1172 が前提の1行を足した）。
 * 残り5箇所は「ただし会話は失われる」とだけ言っていた。
 *
 * ⟹ **1箇所ずつ触ると必ず割れる。** 同じ壊れ方は
 * {@link STALE_TOKEN_RECOVERY_CAVEAT} の doc が既に予言していた——「同じ事実を
 * 2つの口が別の語で呼ぶと、面をまたいで読む人が詰まる」。**注意書きを増やすのでは
 * なく、生成元を1つにして再発を構造で止める。**
 *
 * ## 何を言っているか（3つを1文へ畳んである。順序も含めて意味である）
 *
 * 1. **先に確かめろ**（#1063）——`manager_stop` の断りに未 push・未コミットの実物が
 *    出る。⚠ **前提は助言の手前に置く。**後ろに付くと前提として読まれない
 *    （`tools.test.ts` の順序の歯が、在るかどうかではなく**順序**を測っている）
 * 2. 起こし直せば新しいプロセスなので新しい鍵で走る
 * 3. ⚠ **失われるのは会話だけではない**——そのターンで進行中だった作業も失われる。
 *    これが #914 の名指しへの答えであり、旧文言「ただし会話は失われる」を
 *    置き換えた理由である。**実害は観測済み**（2026-09-16 JST、走行中の委譲3本が
 *    この助言どおり止められ、うち1本は未 push の実装を抱えていた）
 *
 * ## 7箇所目を別の文言で足せないようにしてある
 *
 * `pnpm check:stale-token-restart-advice` が、この定数の外で助言を書くことと、
 * 旧文言「会話は失われる」が復活することの両方で赤くなる。⚠ **門が空振りしない
 * ことは陰性対照で確かめてある**（わざと別文言を足して赤くなることを見た）。
 *
 * ## この定数が言っていないこと
 *
 * - **`lost` の委譲には効かない。** `unpushedWork()` は `manager_stop`（running・
 *   非 force）の断りからしか呼ばれないので、`lost` では 1 の「確かめろ」が空を引く
 *   （#1175 のコメントの実測）。**`lost` に対する守りは、まだ無い**
 * - **逆向きの助言（`manager_start` で起こし直す**な**）は、ここでは畳んでいない。**
 *   条件が違う（あちらは `lost` / 失敗で終わったターン）。⟹ 別 Issue に分けた
 */
...
/**
 * 「直近の枠の事実」を覚えるときの鍵 —— **トークンの身元と枠の種類の組である。**
 *
 * ## なぜ枠の種類（`kind`）だけでは足りないのか
 *
 * `rate_limit_event` が運ぶのは**そのセッションが乗っているアカウントの事実**で
 * あって、デーモン全体の事実ではない。**この repo のアカウントは1つではない**
 * （認証トークンのプール。Issue #393、`.claude/skills/token-pool/SKILL.md`）。
 * しかも鍵が回っても**走行中のセッションには届かない**ので、回転の前後では
 * 「古い鍵で走り続ける層」と「新しい鍵で起きた層」が同時に存在する。
 *
 * ⟹ `kind` だけを鍵にすると、**別々のアカウントの事実が同じ欄を踏み合う。**
 * 壊れ方は両方向に出る:
 *
 * - **配りすぎ** —— 枠が尽きた鍵の `rejected` を、健全な鍵の `allowed` が
 *   踏み消す。次の `rejected` が「新しい遷移」に化けて、一字一句同じ知らせが
 *   もう一度配られる（Issue #1222。1本のマネージャーから同文が4時間43分で
 *   10回以上、台帳と受信箱の両方へ）
 * - **配らなすぎ** —— 同じ `kind` の `rejected` が**別のトークンで**再発しても
 *   {@link usageTransitionOf} が `undefined` を返す（Issue #668。実運用で20分
 *   以上の停止として観測された。2026-09-07）
 *
 * **⛔ 以前はどちらの向きも「気づいた側だけ」を回避してあった** —— 回し手への
 * 受け渡しを遷移の門より手前へ出す、という形である（`manager.ts` / `clone.ts` の
 * `case 'rate_limit'`）。**あれが救ったのは回し手の契機だけで、記憶そのものは
 * トークンを跨いだままだった。**⚠️ **その回避を外さないこと** —— `status` が
 * `'rejected'` でも遷移が立たない回は残るので、手前で渡す必要はいまも在る。
 *
 * ## 身元が取れないときは1つの流れへ倒す
 *
 * `tokenId` が `undefined`（プールを配線していない器・テスト）のときは番人
