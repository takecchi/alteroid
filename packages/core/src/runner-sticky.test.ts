...
describe('manager_id → runner_id の貼り付き（M5 受け入れ基準2 / 3台同時）', () => {
   ...
   *
   * **fencing（#160）は入ったが、これは塞いでいない。** #209（Issue #200 は
   * これで CLOSED）が塞いだのは貸し出しの引き取り側（`mayClaim` が `ambiguous`
   * を拒む）だけで、`Registry#get` 自身の一意性はいまも未解決のままである。
   * `RunnerRegistry#select` の doc（「指名しても片方に固定できない」）と
   * `onSwap` の doc が同じギャップを申し送りしている。
   *
   * **期待値を「どちらの1台か」に固定しないこと。** 内部の走査順に固定すると、
   * `Registry#get` の一意性が解けて正しく1台に決まるようになったときに、
   * 直したのに落ちる（嘘の失敗）か、逆に「順番で決まる」ことを仕様として
   * 守ってしまう。だからここで固定するのは**「両方には行かない」**という、
   * 直した後も成り立つ形だけにしてある。
   */
