 ...
 * 1点だけ。現状の実装（`packages/core/src/clone.ts:1539-1558`）は連結全文を
 * 文字列比較するだけなので、1文字でも変われば全文を丸ごと載せる。実運用の
 * 連結全文は 144,579 文字あり、これが会話履歴に恒久的に残ることでクローンの
 * 費用の最大要因になっている（実測）。
 *
 * このファイルは新規追加であり、`clone.ts` / `clone.test.ts` はどちらも
 * 1文字も変更していない（衝突を避けるため、別マネージャーが `clone.ts` を
 * 並行して持っている）。ここでは `clone.test.ts` の `setup()` と同じ組み立て方
...
describe('クローンの記憶注入（差分のみを載せるべき、という固定したい振る舞い）', () => {
  it('意図的に落ちる: 変わっていない文書の本文は、注入に含まれてはならない', async () => {
    ...
    // 現状の実装（clone.ts の #withFreshMemory）は連結全文を文字列比較するだけ
    // なので、notes が1文字変わっただけで about-me を含む全文が丸ごと載る。
    // ここが落ちること自体が、方針（差分のみ注入）と実装のギャップの証拠である。
  ...
  it('歯1（足場健全性チェック）: 変わった文書の本文は、注入に含まれる（現状の実装でも通る）', async () => {
    ...
    // これが通らなければ、上の「意図的に落ちるテスト」が落ちた理由は
    // 「実装が違うから」ではなく「足場が動いていないから」である可能性が
    // 残る。このテストは通らなければならない（AGENTS.md「テストの足場・
    // スタブ・モックは、動くのに嘘をつく」）。
