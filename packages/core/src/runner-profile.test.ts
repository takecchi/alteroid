 ...
 * 別々のまま通ってしまう。効き方は2経路あり、**片方だけでは足りない**:
 *
 * - `BASH_ENV`: Bash 経由のコマンドに、**1コマンドごとに読み直されて**効く
 *   （走行中のマネージャーへ差し替えが届くのはこちら）
 * - 評価済みの env: MCP サーバのように Bash を経由しない子に効く
 */
...
function makeHost(env: NodeJS.ProcessEnv, fake = fakeSdk()): { fake: ReturnType<typeof fakeSdk> } {
  host = createRunnerHost({
    ...
    queryFn: fake.fn,
    env,
    profile: createProfileVessel({ path: join(dir, 'profile', 'profile.sh') }),
  });
  return { fake };
}
...
describe('マネージャーに効く実行環境プロファイル', () => {
  ...
  it('器を作り直さずに差し替えられる（走行中の仕事を殺さない）', async () => {
    const { fake } = makeHost({ PATH: process.env.PATH ?? '' });
    ...
    // 既に走っている mgr-1 にも、器（`BASH_ENV` が指すファイル）越しに次の
    // コマンドから届く。**同じファイルを指したままである**ことが要点。
