 ...
 * （Issue #1036）。`manager_stop` の断り・`manager_report`・`manager_list` の
 * 3箇所がここを呼ぶ——「この報告は N 分前のもので、いま走っているターンの
 * 中身ではない」を組む文言の生成元を1つに保つ。
 *
 * ## ⚠ を出す条件
 *
 * **「drift が在るとき」＝ `lastReportStatus`（記録した status）といまの
 * `status` が違うとき**（人間の判断。#1036 コメント）。`running` /
 ...
 * `manager_stop` は drift が無い回・比較できない回に自前の generic な
 * 一言で埋める（`tools.ts` 側。ここでは字面を1つに決めない）。
 *
 ...
 * #1037（`manager_stop` を行為の側で断る）のほうであり、この関数はその代わり
 * にはならない——⚠ の1行を増やすことが (2) の答えだとは名乗らない。
 *
 * ## 純関数のまま保つ
 *
 ...
 *
 * `tools.ts` の `describeZombieAge` / `clone.ts` の `formatElapsed` と役目は
 * 似るが、どちらも呼び出し元のファイルに閉じた private 関数で export されて
 * いない。**新しい I/O を増やさない**というこのファイル冒頭の約束の下では、
 * 他ファイルの private 関数を export させに行く（＝依存を増やす）よりも、
 * ここに小さく複製するほうが安い——3つとも「秒→分→時間→日」の丸めだが、
 * 丸め方の粒度がそれぞれ違うので、共通化すると3箇所のどれかが字面を変える
 * ことになる。
 */
