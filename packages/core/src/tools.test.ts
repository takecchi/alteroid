...
describe('一覧は例外なく件数で壊れない（`*_list` の総当たり）', () => {
   ...
   *
   * **フィルタは字下げで切る（#299 で直した）。** 以前は「先頭が
   * `- 総文字数` で始まらない」という、集計行の文言そのものを名指しした
   * 除外だった——節に文書一覧でない`0字下げ`の `- ` 始まり行が増えると、
   * その行まで1件として数えられ、無関係な行が `fiveFieldViolations` に
   * かけられて**偽の失敗**になっていた（黙って通るのではなく、無関係な
   * 理由で赤くなる側）。`renderMemorySize`（tools.ts）が出す文書 entry は
   * 必ず `  - ${doc.slug}: …` の**2字下げ**、集計行 `- 総文字数: …` は
   * **0字下げ**である。だから「先頭が厳密に2字下げの `  - ` である」
   * （`/^ {2}-\s\S/`）を残す条件にした——**測っている中身（id・名前・
   * 作成・更新・概要）ではなく、字下げという直交した軸で切っている**ので、
   * 中身を落とす変異が「フィルタから外れて消える」形にはならない。
   *
   * **⚠️ 窓が狭まっただけで閉じてはいない。** 0字下げの新しい箇条書き
   * （例: 「直近書き込みが多い上位3件:」のような見出し）は字下げで
   * 自動的に除外されるが、**その見出しの下に `  - ` で続く子の行**——
   * つまり文書 entry と同じ2字下げで衝突する行——までは区別できない。
   * 下のテストがこの残る形をそのまま再現している。この節へ `- ` 始まりの
   * 行を足すときは、2字下げと衝突しないか（衝突するなら、この関数を
   * 直すか設計を変えるか）を確認すること。
   */
  ...
  it(
    'extractMemorySizeEntries — 節に0字下げの新しい箇条書き（文書一覧でない行）が増えても、' +
      'その見出し行を一覧の1件として数えない（#299）',
    () => {
      // Issue #299 の「どう壊れるか」の再現例をそのまま使う。「直近書き込みが
      // 多い上位3件:」は0字下げの新しい箇条書きで、文書一覧ではない。
      // extractMemorySizeEntries は self_status の生の文字列を受け取る前提
      // なので、合成した `## 記憶の大きさ` 節を直接渡す（handler を呼ばない）。
      const section = [
      ...
      // 直した性質: 一覧でない見出し行（0字下げ）は1件として数えない。
      expect(
        entries.some((entry) => entry.trimStart().startsWith('- 直近書き込みが多い上位3件')),
      ).toBe(false);
      // 既存の保証: 集計行（0字下げ）も同様に数えない。
      expect(entries.some((entry) => entry.trimStart().startsWith('- 総文字数'))).toBe(false);
      // 実在の文書 entry（2字下げ）は数える——保証を弱めていない。
      expect(entries.some((entry) => entry.includes('doc-a: 題0001'))).toBe(true);

      // **⚠️ doc-b（新しい見出しの子で、2字下げで文書 entry と衝突する行）
      // については主張しない。** これは上のフィルタが残す側の限界であり、
      // doc コメントに書いた「窓が狭まっただけで閉じてはいない」がここである。
    },
  );
