import type { InboxEvent } from './schema.js';

/**
 * クローンの受信箱（docs/architecture.md「同時実行モデル」）。
 *
 * 仕事の起点は人間に限らない（PRD「自律」）。M1 で積まれるのは人間の発言だけだが、
 * 「イベントが積まれ、クローンが一件ずつ取り出す」構造をここで固定しておく。
 * chat 専用の作りにしないための土台であり、M3 でタイマー・外部イベント・発意が
 * 同じ口から入る。
 */
export class Inbox {
  readonly #queue: InboxEvent[] = [];
  readonly #waiters: ((event: InboxEvent | null) => void)[] = [];
  #closed = false;

  get size(): number {
    return this.#queue.length;
  }

  get closed(): boolean {
    return this.#closed;
  }

  /**
   * まだ取り出されていないイベントに、条件を満たすものがあるか。
   *
   * 「同じ合図が処理前に二重に積まれた」を呼び出し側が判断するためのもの。
   * 処理中のものは既に取り出されているのでここには居ない。
   */
  hasPending(predicate: (event: InboxEvent) => boolean): boolean {
    return this.#queue.some(predicate);
  }

  /**
   * まだ取り出されていないものを、**先頭から連続して条件を満たす分だけ**取り出す。
   *
   * `next()` で1件取り出した呼び出し側が、その続きを一緒に読むためのもの
   * （`Clone#pump` の、処理待ちのあいだに積み上がった人間の発言）。
   *
   * **条件を満たさないものに当たったらそこで止める。走査して拾い集めない。**
   * 飛び越えて集めると、間に挟まった別の起点（タイマー・マネージャーからの報告・
   * 外部イベント）より後から届いた発言を先に読むことになり、**並んでいる順に
   * 読む**という約束が崩れる。
   *
   * **⚠️ 「並んでいる順」は「届いた順」ではなくなった。** 人間が待っている合図
   * （`human_message` / `human_answer`）は `push` の `insertAfterLast` で
   * 待ち行列の人間の最後尾へ入るので、**人間以外より前に並ぶ**（人間の決定。
   * `clone.ts` の `CLONE_HUMAN_PRIORITY_ENV_KEY` に逐語がある）。**それでも
   * ここは変えなくてよい** — 並べ替えは `push` の時点で済んでおり、ここは
   * 「いま並んでいる順に、先頭から連続する分だけ」を守るだけである。
   * **飛び越えて拾わない、という約束は同じ強さで生きている。**
   *
   * **待っている取り出し（`#waiters`）には触れない。** ここに居るのは既に
   * 積まれているものだけで、待ち受けが居る状態＝queue は空である。
   */
  drainWhile(predicate: (event: InboxEvent) => boolean): InboxEvent[] {
    const taken: InboxEvent[] = [];
    for (;;) {
      const head = this.#queue[0];
      if (head === undefined || !predicate(head)) return taken;
      this.#queue.shift();
      taken.push(head);
    }
  }

  /**
   * `drainWhile` が次に取るはずの範囲を、**取り出さずに**数える（issue #783）。
   *
   * `drainWhile` と同じ述語で受け、同じ規則で数える —— **先頭から連続して
   * 条件を満たす件数**であり、途中で条件を満たさないものに当たったらそこで
   * 止める（飛び越えて数えない）。`#queue` そのものは1件も動かさない。
   *
   * **`Clone#drainMergeableWithinLimit` が、上限で束を打ち切った直後に
   * 「同じ束へ入るはずの合図があと何件、待ち行列の先頭に残っているか」を
   * 答えるために使う。** `drainWhile` を呼べば同じ答えが取り出せるが、それでは
   * 実際に取り出してしまい、上限の意味が消える —— 数えるだけで済ませたい
   * 呼び出し側のために、取り出さない形をここへ別に持つ。
   */
  countWhile(predicate: (event: InboxEvent) => boolean): number {
    let count = 0;
    for (const event of this.#queue) {
      if (!predicate(event)) break;
      count += 1;
    }
    return count;
  }

  /**
   * まだ取り出されていないものから、**条件を満たすものを全部外す**（issue #1049）。
   * 外したものを**待ち行列に並んでいた順で**返す。
   *
   * ## なぜ要るか —— 消す口が届かない実体がここに在った
   *
   * `inbox_remove_many`（`POST /inbox/remove`）が消すのは `InboxStore`（DB /
   * ファイル）の行だけで、**配達が読むのはこちら**（`Clone#pump` の
   * `for await (const event of this.#inbox)`）である。⟹ 消した後も、既にここへ
   * 積まれた合図はそのまま配られていた —— 道具は「消した」と名乗り、カウンタ
   * （`InboxStore.pending()`）も減るのに、クローンのターンは起き続ける。
   * **「名乗る側」と「配る側」が別の実体を見ている**状態そのものである。
   *
   * ## 述語で受ける理由は `hasPending` / `drainWhile` / `countWhile` と同じ
   *
   * **この器は「どれを消してよいか」を知らない。** 絞り込みの判定は
   * `inbox-backlog.ts` の `matchesInboxRemoveManyFilter` に閉じており、ここへ
   * 起点の種類や消し込みの方針を持ち込むと、**方針が器の中へ散る**（`push` の
   * `insertAfterLast` に「誰が割り込んでよいか」を持たせなかったのと同じ理由）。
   *
   * ## ⚠️ `drainWhile` とは走り方が逆である
   *
   * `drainWhile` は**先頭から連続する分だけ**取り、条件を満たさないものに
   * 当たったらそこで止める（飛び越えて拾わない）。**こちらは待ち行列を全部
   * 走査して、当たったものを飛び越えて外す。** 逆にしてあるのは、消す対象が
   * 待ち行列の中に**散らばっている**からである —— 絞り込みで選んだ id 群は
   * 連続している保証が無く、「先頭から連続する分だけ」では1件も外せない回が
   * 普通に出る。**順序の約束は壊れない**（残ったものの相対順序は1文字も
   * 動かない。外すだけで並べ替えないので、`drainWhile` が守っている「並んで
   * いる順に読む」はそのまま生きている）。
   *
   * ## 取り出しの最中に呼んでも安全である
   *
   * **`next()` は先頭を `shift` するだけで、`await` を跨いで添字も反復子も
   * 保持しない。** ⟹ 待ち行列のどこを splice しても、`for await` が「飛ばす」
   * ことも「二度読む」ことも起こらない（歯が在る —— `inbox.test.ts` の
   * 「取り出しの最中に外しても、イテレータが飛ばしも二度読みもしない」）。
   *
   * **待っている取り出し（`#waiters`）には触れない。** 待ち手が居る状態＝
   * 待ち行列は空なので、外すものがそもそも無い（`drainWhile` と同じ）。
   *
   * ## ⛔ ここに居ないものは外せない。それは呼び出し側の仕事である
   *
   * **既に取り出されたものはここに無い。** いま処理中の1件と、枠（利用上限）で
   * 保持されている分（`Clone` の `#deferred`）がそれである。**前者は取り消せ
   * ない**（もう走っているターンを止めれば掛かった分が捨てられる）が、**後者は
   * 解除で待ち行列の先頭へ戻ってくるので、呼び出し側が一緒に落とさなければ
   * 配られてしまう** —— その責任は `Clone#dropQueuedInboxEvents` が持つ。
   *
   * ## 閉じていても投げない（`push` / `unshift` とは違う）
   *
   * あの2つが閉じた受信箱で投げるのは、**積んだものが誰にも読まれず静かに
   * 失われる**からである。外す側にその危険は無い —— 減らす操作なので、閉じた
   * 後に呼ばれても失われるものが増えない。`stop()` の後始末（`for await` が
   * 残りを吐き出しているあいだ）に消す口が走っても、拒む理由が無い。
   */
  removeWhere(predicate: (event: InboxEvent) => boolean): InboxEvent[] {
    const removed: InboxEvent[] = [];
    // **後ろから外す。** 前から回して splice すると、外した分だけ後続の添字が
    // 手前へ詰まるので、1件外すごとに次の1件を読み飛ばす（古典的な取りこぼし）。
    for (let i = this.#queue.length - 1; i >= 0; i--) {
      const queued = this.#queue[i];
      if (queued === undefined || !predicate(queued)) continue;
      this.#queue.splice(i, 1);
      removed.push(queued);
    }
    // 後ろから外したので、集めた配列は並びが逆になっている。**待ち行列に並んで
    // いた順へ直して返す** —— 呼び出し側は跡（日誌の1行）を書くためにこれを
    // 読むので、順が逆だと「何が配られるはずだったか」が読めなくなる。
    return removed.reverse();
  }

  /**
   * イベントを積む。**既定は末尾**（純粋な先入れ先出し）。
   *
   * ## `insertAfterLast` — 割り込ませる口
   *
   * 渡すと、**その述語に最後に一致した要素の直後**へ入れる。1つも一致しなければ
   * **先頭**へ入れる。省略すれば末尾で、これまでと1文字も変わらない。
   *
   * **「誰が割り込んでよいか」はここに持たない。** 述語を受け取るだけにしてある
   * ので、この器は「人間」も「マネージャー」も知らない。判断は呼び出し側
   * （`Clone#post`）が持つ — 器が起点の種類を知り始めると、**優先順位の方針が
   * 器の中へ散る**（`hasPending` / `drainWhile` を述語で受けているのと同じ理由）。
   *
   * **この形が「人間どうしは FIFO」を同時に満たす。** 述語を「人間起点か」に
   * すると、新しい人間の発言は**既に並んでいる人間の後ろ**へ入る（追い越さない）
   * 一方、人間より後ろに居る人間以外は全部飛び越す。**2つの規則を1つの操作で
   * 表しているので、片方だけ実装される形が作れない。**
   *
   * ## 効かない場面がもう1つある — `unshift` の直後
   *
   * **後から来た人間は「既にいる人間より前」へは決して行かない**（述語に最後に
   * 一致した要素の直後、なので）。**つまりその人間より前に在るものは、何であれ
   * 飛び越えない。** そして**人間以外が人間より前に来られる経路は `unshift` だけ**
   * である（`push` は必ず人間を人間以外より前へ置く）。**発生源は1か所で、それが
   * 唯一である** — ここは構造で閉じているので、他を探さなくてよい。
   *
   * 起きる条件は、枠の解除で保持分が先頭へ戻された後（`clone.ts` の `#pump` の
   * 解除ブロック）、その時点で**人間の合図が既に待ち行列に並んでいて**、さらに
   * **後から別の人間の合図が届いたとき**。実測（`dist` の `Inbox` を直接動かした）:
   *
   * ```
   * 人間が既に並んでいる  : held1 → held2 → H1 → H2 → M1   ← H2 は held を飛び越えない
   * 人間が並んでいない    : H2 → held1 → held2 → M1        ← H2 は飛び越える
   * ```
   *
   * **大きさ**: 後から来た人間は**保持件数 × 1ターン**待つ。1ターンは 20〜120 秒
   * （クローンの実測、2026-08-22）。**枠がまだ閉じていれば保持分は短絡する**
   * （ターンを回さない）ので速い。開いていれば本物のターンとして回る。
   *
   * **直さないのは「直せない」からではない。** H2 を held の前へ出すには、H1 より
   * 前へ出す（**人間どうしの FIFO が壊れる**）か、held を後ろへ下げる（**`unshift`
   * の復元順が壊れる**＝「配り直しは到着順のまま」の歯が落ちる）しかない。
   * **どちらも人間とクローンが明示的に守れと決めたものである。** 直そうとする人は、
   * 先にどちらを壊すのかを決めること。
   *
   * **待ち手が居るときは順序の話にならない。** 待ち行列が空だから待っているので、
   * そのまま渡す（＝クローンが暇なときは、この口は何もしない）。
   */
  push(event: InboxEvent, insertAfterLast?: (queued: InboxEvent) => boolean): void {
    if (this.#closed) throw new Error('受信箱は既に閉じている');
    const waiter = this.#waiters.shift();
    if (waiter) {
      waiter(event);
      return;
    }
    if (insertAfterLast === undefined) {
      this.#queue.push(event);
      return;
    }
    // **後ろから探す。** 「最後に一致したものの直後」なので、前から探して
    // 数えるより、末尾から最初の一致を見つけるほうが同じ答えを短く出せる。
    let at = 0;
    for (let i = this.#queue.length - 1; i >= 0; i--) {
      const queued = this.#queue[i];
      if (queued !== undefined && insertAfterLast(queued)) {
        at = i + 1;
        break;
      }
    }
    this.#queue.splice(at, 0, event);
  }

  /**
   * 取り出したイベントを**待ち行列の先頭へ、渡した順のまま**戻す。
   *
   * **`push` では代用できない。** 戻したいのは「もう取り出したが、実はもっと
   * 早く届いていた」ものなので、末尾へ積むと到着順（FIFO）が壊れる。使うのは
   * クローンが枠（利用上限）の解除をするときで、**保持していた合図と、いま
   * 取り出したばかりの合図を、到着順に並べ直して受信箱へ返す**
   * （`clone.ts` の `#pump` 先頭の解除。あちらの doc に経緯がある）。
   *
   * **待っている取り出しが居れば先頭から渡す。** いまの唯一の呼び出し元
   * （`#pump` の本体）は自分が処理中なので待ち手は居ないが、居ないことを
   * 前提に黙って queue へ積む形にすると、将来別の場所から呼ばれたときに
   * 「積んだのに誰も起きない」＝**静かに止まる**（AGENTS.md「静かに失敗する
   * 道具」）。順序は queue の並びが決めるので、ここで渡しても FIFO は崩れない。
   */
  unshift(events: readonly InboxEvent[]): void {
    if (this.#closed) throw new Error('受信箱は既に閉じている');
    this.#queue.unshift(...events);
    while (this.#waiters.length > 0 && this.#queue.length > 0) {
      const waiter = this.#waiters.shift();
      const next = this.#queue.shift();
      if (waiter !== undefined && next !== undefined) waiter(next);
    }
  }

  /** 次のイベントを待つ。閉じられて空になったら null。 */
  async next(): Promise<InboxEvent | null> {
    const queued = this.#queue.shift();
    if (queued !== undefined) return queued;
    if (this.#closed) return null;
    return new Promise<InboxEvent | null>((resolve) => {
      this.#waiters.push(resolve);
    });
  }

  /** 待っている取り出しを全部起こしてから閉じる。 */
  close(): void {
    if (this.#closed) return;
    this.#closed = true;
    while (this.#waiters.length > 0) {
      this.#waiters.shift()?.(null);
    }
  }

  async *[Symbol.asyncIterator](): AsyncGenerator<InboxEvent> {
    for (;;) {
      const event = await this.next();
      if (event === null) return;
      yield event;
    }
  }
}
