  ...
  async #onEvent(event: RunnerEvent): Promise<void> {
    ...
    switch (event.type) {
      ...
      case 'report': {
        ...
        // 早期リターン（このすぐ上）を通らなかった回——つまりこの report は
        // 完遂した報告として台帳へ書かれる。古い畳んだ本文を残すと、次に
        // `manager_report` を読む側が「これは畳まれた途中経過だ」と誤読する
        // （`lastFailure` / `lastUnreported` を応答として終わった回で消すのと
        // 同じ理由——`schema.ts` の `lastFoldedTurn` の doc）。
