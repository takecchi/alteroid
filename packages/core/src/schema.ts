...
export const permissionRuleSchema = z.object({
  ...
  /**
   * **どの資格でこの口を通ったか。「誰が足したか」ではない。**
   *
   * 値は `operator`（実行環境の持ち主の token を提示した）か `account:<id>`
   * （ログインしたアカウント）である。
   *
   ...
   * ## では何が担保か
   *
   * **禁止ではなく記録である。** クローンの道具に書き口を作っていないので、
   * クローンがここを叩くには**自分の判断で `Bash` を使って器の資格を読み出す**
   * という手順を踏むことになり、その手順は `PostToolUse` フックで日誌に残る
   * （`clone.ts` の `#onPostToolUse`）。**「できない」のではなく「してはいけない」で
   * あり、後から人間が読んで否定できることが最終承認の実体である**（north_star）。
   *
   * **ここを `'human'` 固定にしていた版があったが、それは嘘だった** — クローンが
   * 叩いても `'human'` と記録され、記録の側から誰が足したか読めなくなっていた。
   */
  grantedBy: z.string().min(1),
  /** なぜ許したか（人間が書く1行）。**無くてもよいが、在れば後から否定できる。** */
