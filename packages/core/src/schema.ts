...
export const jobLeaseSchema = z.object({
   ...
   * セッションへ黙って混ざる。
   *
   * **貸し出しを返した後に貸し直すと 1 から数え直す**（返す＝この欄そのものを消す）。
   * 単調でなくてよいのは、世代が意味を持つのは**runner のセッションが生きている間
   * だけ**だからである — 貸し出しを返す契機（`closed` / 止まったと確かめた停止）は
   * どちらも「そのセッションは終わった」の印で、終わったセッションと一緒に runner 側の
   * 記憶も消える。**逆に言えば、セッションが生きているうちに返してはいけない。**
   */
