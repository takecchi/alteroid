...
/**
 * 開いている実行許可1件（SDK の許可規則そのもの）。
 *
 * **これは PRD「権限境界」の一覧ではない。** 混同すると正典を踏むので、2つの層を
 * 分けて読むこと:
 *
 * | | 上の層: 権限境界 | 下の層: ここ |
 * |---|---|---|
 ...
 * | 正典 | PRD「権限境界」— **一覧を作るな。記憶として持続させろ** | north_star 禁止2 — **方針は設定で開けられなければならない** |
 *
 * 上の層は触らない（クローンは相変わらず記憶から判断する）。ここが変えるのは
 * **クローンが判断した結果を器が実行させてくれるかどうか**だけである。
 *
 * **なぜ要るか。** 既定の `permissionMode: 'auto'` では SDK のモデル分類器が
 * その場で拒否を決め、`canUseTool` は呼ばれない（`runner.ts` の `permission_denied`
 * の注記）。だからクローンが記憶に根拠を見つけて「これはやってよい」と判断しても、
 * 価値観を1つも知らない分類器がその手前で落とす。**人間が自分の PC で Claude Code
 * を開けば一度「常に許可」を押せば以後通るのに、クローンにはその操作が存在しない**
 * ＝ north_star 禁止1（デグレード禁止）。ここはその「一度押す」を作る器である。
 *
 * ## 不変条件1: `allow` しか持たない
 *
 * **`deny` と `ask` を持たせないこと。** 列を作らない。API に口を作らない。
 * 「便利だから」で後から足さない。AGENTS.md の地雷表が禁じているのは
 * 「**確認が要る**行為の一覧（`permissions.yaml` 的なもの）」＝ *ask / deny* の側で、
 * それは判断を設定へ移す形である。ここが持つのは「人間が既に許した行為」だけで、
 * 判断は1つも移していない。**足した瞬間に、これは正典が名指しで禁じたものになる。**
 *
 * 何かを止めたくなったら、止める場所はここではない — **方針**（記憶・システム
 * プロンプト）と**実行環境の境界**（配る認証情報の範囲・サンドボックス）である。
 *
 * ## 不変条件2: 「どの層に効くか」で鍵付けしない
 *
 * **層の列を作らないこと。** ここが効くのはクローンの本セッションだけである。
 * マネージャー層にも同じ穴はあるが、そちらへ広げるのは別の判断であり、広げるなら
 * そのとき人間が決める。
 *
 * 理由は `storage-pg` の `env_profile` の DDL コメントが先回りで書いている —
 * 「用途ごとに行を増やす形にしないのは、増やせる形にした瞬間に**どの行がどの層に
 * 効くかの対応表**が生まれ、**それが権限の一覧に化けるからである**」。この表は
 * まさにその形になりうる。
 *
 * ## ⚠️ これが直さないもの
 *
 * **ここが直すのは「*許可*が原因の拒否」だけである。** 拒否の原因が許可の層でないなら、
 * 規則を何本足しても通らない。
 *
 * **そして `auto` では、足した規則が黙って効かないことがある。** 出荷されている判定
 * 実装を静的に読んだ結果（`decision_reason_type` の分岐 `BOe` / `N1t` / `B4n`）、
 * `auto` のときは**危険と判定された Bash の allow 規則が、照合される前に候補から
 * 除外される** — インタプリタ・遠隔実行系（`python` / `node` / `ssh` /
 * `kubectl exec` など）がそれに当たり、設定 `autoMode.classifyAllShell` が真なら
 * **Bash の allow 規則は全部**除外される。除外された規則は一致しようがないので、
 * **人間は許可したつもりなのに一度も効かない。**
 *
 * 放っておくと「クローンが許可が足りないと読む → 人間が足す → それでも通らない →
 * 誰も原因に近づかない」＝**効かない許可を積み上げる装置**になる。だから
 * `clone.ts` の `#noteDenial` が、**既に許可済みの道具が止められたこと**を検出して
 * 目立たせる（そのとき原因が許可の一覧でないことは確定する）。
 *
 * ## ⚠️ 規則は見た目より広い
 *
 * 照合は**コマンド文字列の前方一致**である（`aIl`: `command.startsWith(prefix + ' ')`。
 * 長さの上限も、後続に何が来るかの制限も無い）。したがって `Bash(gh pr merge:*)` は
 * **`gh pr merge ` で始まる任意の長さの命令**に一致する — ヒアドキュメントを含む
 * 巨大な命令もそのまま一致する。
 *
 * **さらに、規則で allow が確定した呼び出しは分類器も安全性検査も通らない**（判定は
 * 規則 → 一致すれば即 return、という順序である）。つまり規則を1本足すことは、その
 * 前方一致に当たる命令の**中身の検査を丸ごと外すこと**でもある。狭めはしないが、
 * **これを「1つのコマンドだけを許した」と読まないこと。**
 */
export const permissionRuleSchema = z.object({
   ...
   *
   * **器はこの文字列の中身を解釈しない。** 何が広くて何が狭いかを判定する表を
   * ここに持つと、それ自体が「行為の一覧」になる（不変条件1）。広さを人間へ
   * 見せるのは表示側の仕事で、器が持つのは人間が書いた文字列そのものである。
   */
  rule: z.string().min(1),
  grantedAt: isoDateTime,
  /**
   * **どこを通って足されたか。「誰が足したか」ではない。**
   *
   * 値は3つある:
   *
   * - `clone` — **クローンが自分の道具（`permission_grant`）で開けた。**
   *   人間の代わりにクロードが判断して開ける経路であり、これは人間の決定である
   *   （2026-08-22。`design-notes/dynamic-permissions.md` §12 に逐語がある）。
   *   判断の根拠は日誌の `grounds` に必ず在る
   * - `operator` — 実行環境の持ち主の token を提示して HTTP を通った
   * - `account:<id>` — ログインしたアカウントとして HTTP を通った
   *
   * **`clone` だけは肯定的に観測できる**（クローンの道具からしか出ない値であり、
   * HTTP の本文からは名乗れない）。残る2つは次項のとおり、人間だとは言えない。
   *
   * ## ⚠️ `operator` は「人間が足した」を意味しない
   *
   * `operator` の資格は `~/.alteroid/state/daemon.json` を読めることでしかない
   * （`auth.ts` の `isOperator`: 「守っているのはファイルの許可であって、新しい秘密
   * ではない」）。**そしてクローンの手はデーモンと同じ器の中にある。** クローンは
   * `Bash` を持つので、そのファイルを読んで `curl` でこの口を叩ける。**そのとき
   * ここには人間が CLI から足したときとまったく同じ `operator` が入る。**
   *
   * さらに、認証を設定していない構成（`authPlan.enabled` が偽）では**すべての
   * 要求が `operator` になる**ので、区別はもっと粗い。
   *
   * **つまり「クローンが自分で足していないこと」は、この列では確かめられない。**
   * 確かめられるのは「どの資格が提示されたか」までである。**器の中からの到達と
   * 人間の入口からの到達を区別する仕組みは無い**（作れば入口ごとの能力差になり、
   * PRD「入口の等価性」と正面から当たる）。
   *
   * ## では何が担保か
   *
   * **禁止ではなく記録である。** 開けた・外したは必ず日誌に残り（`decision`）、その
   * `grounds` には**なぜ開けたか**が入る。**「できない」のではなく「後から人間が読んで
   * 否定できる」ことが最終承認の実体である**（north_star）。
   *
   * **クローンに道具を渡したことで、この列の観測はむしろ良くなった。** 道具が無かった
   * 頃、クローンが `Bash` で `curl` を叩けばここには `operator` が入り、人間が足した
   * ものと1文字も違わなかった。いまは `clone` と記録されるので、**人間の分と混ざらない。**
   *
   * **ここを `'human'` 固定にしていた版があったが、それは嘘だった** — クローンが
   * 叩いても `'human'` と記録され、記録の側から誰が足したか読めなくなっていた。
   */
  grantedBy: z.string().min(1),
  /**
   * なぜ許したか（1行）。**無くてもよいが、在れば後から否定できる。**
   *
   * **クローンが開けたときの根拠は、ここではなく日誌の `grounds` が本体である**
   * （`permission_grant` の `grounds` は必須。ここは一覧で読むための覚書）。
   */
