 ...
 *
 * **長いことの代償は、経路が分かれたときに二重実行が起きうる窓がこの長さになる
 * ことである。** 器が古い器を殺す構成（Railway / compose）では `LEASE_DRAIN_MS` の
 * 側が先に効くので、この長さは「器が殺してくれなかった場合の上限」である。
...
export function judgeLease(input: {
  ...
  // 壊れた時刻で「まだ握られている」と言うと、直せないまま止まる。判定しない側へ倒す。
  const ttlDeadline = Number.isNaN(seenAt) ? now : seenAt + lease.ttlMs + LEASE_MARGIN_MS;
