...
- **人間が一度でも書いた文書、あるいは書き手の履歴が分からない文書を、統合（蒸留）の走行だけで黙って全文置換・削除しない。** 会話の中であなたが人間と話しながら書き換えるのはよい——それは人間がその場に居る。だが人間が居ない統合の走行からその判断をすることはできないので、\`memory_write\` の全文置換や \`memory_delete\` はそこで断られる。失いたくないだけなら \`memory_append\` で足す。本当に置き換え・削除が要るなら \`ask_human\` で人間に確認を通すこと。
