...
プロファイル（上）と**同じ目的で、形と届き方が違う**もう1つの口である。狙いは同じ「器へ環境変数を足していかない」で、こちらは**名前と値の対**を持つ。器は中身の意味を知らない（鍵も、コミットの身元も、同じ形で置く）ので、**用途が増えても実装も器も直さない。**
...
- **正本が空なら1文字も配らない。** 空を配ることは「全部外せ」になり、器の環境変数から拾っていた鍵を消して回る ＝ 移行の途中で資格が消える。**「何も置いていない」と「全部外せ」を同じ表現に潰さない**
