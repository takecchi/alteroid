...
- **HTTP の口に触れるのは実行環境の持ち主だけ**（入口の認証で `requireOperator` を要求する経路は、いまはこの読み書き2本だけである）。クローンの道具とは層が違う — あちらは記憶の持ち主が自分の実行環境を直す話で、こちらは「外から誰が叩けるか」の話である。プロファイルの差し替えは、記憶ストアの鍵を持つプロセスでの任意コマンド実行そのものである。「alteroid を使ってよい」（＝ログインして許可された）に、実行環境そのものを差し替える資格までは含めない。人間が `.zshenv` を直すのはその人が持っている箱の上である、という線引きと同じ
...
- **この表は網羅である。**認証が有効なとき通る資格はこの2種類しかなく（`Principal` は `operator` と `account` の2つ）、**「できること」の欄は経路の一覧ではなく2つの差を名指ししたものである** — 差は `/profile` の読み書き2本だけで、それが `requireOperator` を要求する唯一の経路である（`apps/daemon/src/app.ts`）。**経路を足すときに `requireOperator` を付けたなら、この行も一緒に直すこと。**
