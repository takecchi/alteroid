...
- **触れるのは実行環境の持ち主だけ**（入口の認証で `/access/*` と同じ資格）。プロファイルの差し替えは、記憶ストアの鍵を持つプロセスでの任意コマンド実行そのものである。「alteroid を使ってよい」（＝ログインして許可された）に、実行環境そのものを差し替える資格までは含めない。人間が `.zshenv` を直すのはその人が持っている箱の上である、という線引きと同じ
