...
**未完のフェーズ**の成果物・受け入れ基準・地雷・**順序の理由**は、**凍結した** [roadmap.md](./roadmap.md) に**記録として**在る（2026-08-22 に凍結。消しても要約してもいない）。**ただし実装の起点はそちらではなく台帳である** — クローンの `commitment_list` が「いま何に着手すべきか」を答える。roadmap が答えるのは「凍結時点で何が未完だったか」だけである。
